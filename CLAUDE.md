...
Current phase: **Phase 2.1B** ✅ Complete
...
Next recommended step: **Verify Dashboard per-role (2.1B): login sebagai user `sales`/`operations` (cek KPI personal + Aktivitas Saya + SalesPerformance/LeadsBySource hilang) vs manager (data tim). Staging pending: tabel `sales_calls` (2.0Z — KPI Call=0 sampai dibuat), 3 kolom `quotations` (2.1A), kolom `won_reason`/`visit_type` (2.0X).**
