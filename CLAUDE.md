...
Current phase: **Phase 2.1D-hotfix** ✅ Complete
...
Next recommended step: **Verify Master Customer 4 sub-menu (2.1D): sidebar CRM → Master Customer expand → klik Customer MSI/JCI/SOA/Free Agent (cek header+filter terkunci per entitas, dropdown entitas hilang), klik row → detail. Catatan data: filter MSI/JCI/SOA butuh `customers.source_company_id` terisi; Free Agent butuh `status='free_agent'`. Pending lain: verify Dashboard per-role (2.1B); staging `sales_calls` (2.0Z), 3 kolom `quotations` (2.1A), `won_reason`/`visit_type` (2.0X), opsional `contract_no`/`last_activity_at` (2.1C).**
