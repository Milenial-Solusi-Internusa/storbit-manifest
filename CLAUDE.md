...
Current phase: **Phase 2.1F** ✅ Complete
...
Next recommended step: **Verify CustomerListPage Lovable port (2.1F): buka Master Customer (per entitas) → cek 4 stat cards (icon+hint, dari filtered), filter bar (search/Tier/Status), tabel (avatar navy, PIC avatar, badge pill+dot, payment terms, last activity), row klik → detail, eye/pencil aksi, Export CSV, Tambah Customer (form). Catatan data: "Last Activity" fallback ke created_at jika `last_activity_at` belum ada; Payment Terms butuh `payment_term` join. Pending lain: verify CustomerDetailPage (2.1E), 2.1D sub-menu, Dashboard 2.1B; staging `sales_calls` (2.0Z), 3 kolom `quotations` (2.1A), `won_reason`/`visit_type` (2.0X).**
