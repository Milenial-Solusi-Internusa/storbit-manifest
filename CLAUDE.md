...
Current phase: **Phase 2.1G-hotfix** ✅ Complete
...
Next recommended step: **Verify CustomerDetailPage Lovable port (2.1G): klik customer → cek header card (avatar/badges/Credit Limit box), 6 tab underline, Info/Komersial grid 2-kolom, History Visit (expand MOM/follow_up), BANT (BantScoreBar + 7 kriteria + pipeline), **Health Score** (gauge + 5 komponen + rekomendasi + banner heuristik), Notes inline edit, Edit modal, Hapus role-gate. Catatan: Health Score = heuristik sementara (TODO auto-calc); butuh customer ber-`prospect_id` utk BANT/visit/health penuh. Pending lain: verify CustomerListPage (2.1F), 2.1D sub-menu, Dashboard 2.1B; staging `sales_calls` (2.0Z), 3 kolom `quotations` (2.1A), `won_reason`/`visit_type` (2.0X).**
