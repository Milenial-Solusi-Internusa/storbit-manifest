...
**Current phase: 2.10K** ✅ Complete

Recent (terbaru → lama; detail granular di git history & `PROGRESS.md`):
- **2.10K** — Notification bell aktif + producer notifikasi. 3 file. **App.jsx (bell UI):** state `notifications`/`unreadCount`/`notifOpen`; fetch `notifications` (user_id=me, is_read=false, limit 20) mount + 60s interval; badge orange (>99→"99+"); dropdown (Lucide icon per event_type, title/body/relative-time, "Tandai semua dibaca", klik→mark-read+navigate ref [`hrga_request`→hrga-semua-request, `activity`→crm-calls], empty state, Escape/overlay close). Fix bug badge Pending Approval: status `['submitted','under_review']`→`['submitted','in_progress']`. **Producer (fire-and-forget, skip self/null, `company_id` NOT NULL disertakan):** ActivitiesPage CREATE→assignee (`activity_assigned`), mark-done→creator (`activity_done`); useHrgaRequests submit→approver(s) di Step 7 existing (`hrga_approval_needed`, +`type_name` ke rtRow select), submitApproval terminal→requester (`hrga_approved`/`hrga_rejected`, +`requester_id,subject` ke req select). RLS `notifications_read/insert/update` sudah ditambah via SQL Editor. Build clean (2550 modules, 1.26s). **Belum: tes manual runtime.**
