...
Current phase: **Phase 2.1E** ✅ Complete
...
Next recommended step: **Verify CustomerDetailPage rebuild (2.1E): klik customer dari list → cek visual clone Asset (breadcrumb, header card avatar+badges, in-card tab underline), 5 tab (Info/Komersial/History Visit/BANT/Notes), Edit modal, Hapus role-gate, inline Edit Notes. Catatan DB: BANT/History butuh customer ber-`prospect_id` (FK `customers_prospect_id_fkey`); kolom `contract_no`/`last_activity_at` mungkin belum ada → "—". Pending lain: verify 2.1D sub-menu, Dashboard 2.1B; staging `sales_calls` (2.0Z), 3 kolom `quotations` (2.1A), `won_reason`/`visit_type` (2.0X).**
