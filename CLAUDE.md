...
Current phase: **Phase 2.1C** ✅ Complete
...
Next recommended step: **Verify Master Customer (2.1C): list → klik row → detail page (5 tab), Edit modal, Hapus (role gate), History Visit/BANT (perlu customer ber-`prospect_id`), inline Edit Notes. Opsional: tambah kolom `contract_no`/`last_activity_at` di `customers` (staging) agar tab Komersial terisi. Pending lain: verify Dashboard per-role (2.1B); staging `sales_calls` (2.0Z), 3 kolom `quotations` (2.1A), `won_reason`/`visit_type` (2.0X).**
