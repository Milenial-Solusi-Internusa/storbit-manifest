...
// HRGA Request — Arsip (archived / completed / cancelled requests).

import { useState, useCallback } from 'react';
import { Archive, Eye } from 'lucide-react';
import {
  D, Card, HrgaStatusBadge, TypePill, Avatar,
  FilterBar, FilterDropdown, Pager, EmptyRow, LoadingRow,
  fmtDate, fmtRupiah, HRGA_TABLE_CSS,
} from '../HrgaShared';
...
export default function ArsipPage({ onOpenDetail }) {
  ...
  // Client-side filter to archived statuses
  const archiveStatuses = ['approved','completed','rejected','cancelled'];
  let rows = data.filter(r => archiveStatuses.includes(r.status));
    ...
    <div style={{ fontFamily:"'Plus Jakarta Sans', system-ui, sans-serif", color:D.ink }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:16, marginBottom:20, flexWrap:'wrap' }}>
        <div>
          <nav style={{ display:'flex', alignItems:'center', gap:6, fontSize:12.5, color:D.inkFaint, marginBottom:8 }}>
            ...
            <span style={{ color:D.inkSoft, fontWeight:600 }}>Arsip</span>
          </nav>
          <h1 style={{ fontSize:22, fontWeight:800, letterSpacing:'-.4px', margin:0 }}>Arsip Request</h1>
          <div style={{ fontSize:13.5, color:D.inkSoft, marginTop:4 }}>Request yang sudah selesai, ditolak, atau dibatalkan</div>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <span style={{ display:'inline-flex', alignItems:'center', gap:6,
            fontSize:12.5, fontWeight:600, color:D.inkSoft, padding:'6px 12px',
            borderRadius:8, background:D.bgAlt, border:`1px solid ${D.line}` }}>
      ...
      <Card>
        <FilterBar search={rawSearch} onSearch={handleSearch}
          placeholder="Cari no. dokumen, keperluan…" onRefresh={refresh}>
          ...
          <div style={{ overflowX:'auto' }}>
            <table className="hg-tbl">
              ...
              <tbody>
                ...
                {!loading && rows.length === 0 && <EmptyRow colspan={9} message="Tidak ada arsip." />}
                {!loading && rows.map(row => (
                  <tr key={row.id} style={{ cursor:'pointer', opacity:.85 }}
                    onClick={() => onOpenDetail?.(row.id)}>
                    <td className="stick-l" style={{ left:0 }} onClick={e => e.stopPropagation()}>
                      <input type="checkbox" style={{ width:15, height:15, cursor:'pointer', accentColor:D.accent }} />
                    </td>
                    <td className="stick-l" style={{ left:34 }}>
                      <span style={{ fontFamily:"'IBM Plex Mono', monospace", fontSize:12.5, fontWeight:600, color:D.inkSoft }}>
                        {row.document_no}
                      </span>
                    </td>
                    <td>
