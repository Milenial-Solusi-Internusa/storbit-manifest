...
- **`schema_snapshot.sql` STALE untuk Tahap A/B** (definisi `prf_cost_items` di snapshot masih 10 kolom; body `save_prf_pricing` masih versi lama) → sumber kebenaran sementara = file migrasi. **`pg_dump` sudah DIRENCANAKAN Den sebelum commit — bukan utang terbuka.** Skema + guard: `03_DATA_MODEL` (`prf_cost_items`, `prf.exchange_rates`). Utang terkait: `08_TECH_DEBT` **TD-120**/**TD-121** (+TD-113/TD-114 sekelompok), **TD-115**, **TD-122** (Tahap D).
