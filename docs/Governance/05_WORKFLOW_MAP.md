...
>
> **Diperbarui 2026-07-21 (Halaman Master Vendor — Procurement; FE-only, NOL perubahan DB/RLS) — +section "Procurement — Master Vendor".** Tabel `vendors` akhirnya punya **pintu masuk UI** (leaf `proc-vendor-list` yang dulu `soon:true` kini aktif): list company-scoped + form tambah/edit 18 field + **"Nonaktifkan" = soft delete `deleted_at`** (arsip, nol `.delete()`). `company_id` diisi dari `profile.company_id` (tak diinput manual); `currency_code` = dropdown `currencies` (ber-FK); kode kembar per company → `23505` → toast eksplisit; `bank_account` (SENSITIVE) hanya ditarik saat Edit, tidak di query list. **Gate menu = cermin persis RLS `vendors_*`** (`has_role('procurement')` + tujuh kode `is_manager_or_above()`; **`sales` dikecualikan**) — tapi karena `canSeeMenuItem` memakai **role primer**, user multi-role (`sales`+`procurement`) **tak melihat menu meski RLS mengizinkan** → gate FE lebih ketat dari RLS (**TD-105**); `supervisor` dalam daftar = mati (**TD-106**). ⚠️ **Vendor BELUM tersambung ke PRF/PR/PO** — ini master data CRUD, bukan alur transaksi. ~~⚠️ NOL tes runtime (halaman maupun gate).~~ **[KOREKSI 21 Jul 2026 — kini SEBAGIAN TERUJI]** dengan login **procurement asli** (SCM Master, bukan super_admin): menu muncul, leaf aktif (tanpa `soon`), halaman terbuka tanpa error konsol, dan **list company-scoped terbukti benar** (user MSI melihat 1 vendor MSI `MSI-0023`, bukan 4 seed SOA) → **jalur SELECT + INSERT RLS `vendors` baru sudah terbukti runtime**. **Belum teruji: edit tersimpan & "Nonaktifkan" (soft delete)** — dan justru soft delete inilah yang diduga memicu `error 42501` (lihat bullet "PENYARINGAN ARSIP" di section ini). **+21 Jul 2026: penyaringan vendor terarsip sedang dipindah dari RLS ke query kode** (dropdown GRN Penerimaan Barang sudah ikut `deleted_at IS NULL`; pencabutan RLS **belum** dijalankan). → `04_ROLE_PERMISSION_MATRIX` (baris Master Vendor) + `PROGRESS.md` 2026-07-21 + `03_DATA_MODEL` entri `vendors` + `08_TECH_DEBT` TD-105/TD-106.
...
## Procurement — PRF (Price Request Form) Flow (Fase 1+2 LIVE; Pricing Answer LIVE 20 Jul; Buat Quotation dari PRF 20 Jul [positive-case belum tes runtime]; Fase 3 belum)
...
## Procurement — Master Vendor (CRUD master data; halaman LIVE 21 Jul 2026, tes runtime SEBAGIAN)

> Pintu masuk UI pertama untuk tabel `vendors`. **Master data murni — BUKAN alur transaksi.** ⚠️ **Vendor BELUM tersambung ke PRF** (multi-vendor `prf_cost_items` tidak ada — lihat `PROGRESS.md` 2026-07-21) dan **belum tersambung ke PR/PO** (belum dibangun). Jangan diklaim lebih dari ini.
...
[procurement / manager+] Procurement → Vendor Management → Vendor List   ← leaf `proc-vendor-list` (dulu `soon:true`)
   ...
   → "Nonaktifkan" (per-baris) → ConfirmModal → UPDATE deleted_at = now()   ← ARSIP, bukan hard-delete
        baris hilang dari daftar (query filter deleted_at IS NULL); TIDAK ada UI pemulihan
```

- **Konsumen vendor yang SUDAH ADA (jangan lupa saat mengubah halaman ini):** dropdown vendor di **Penerimaan Barang** (`PenerimaanBarangPage.jsx:332`, modul Inventory). Halaman Master Vendor = **penulis pertama**; Penerimaan Barang = pembaca lama. **[21 Jul 2026]** dropdown itu kini menyaring **`company_id` + `is_active = true` + `deleted_at IS NULL`** (satu baris ditambahkan) → vendor yang sudah diarsipkan tak lagi bisa dipilih saat penerimaan barang.
- **PENYARINGAN ARSIP sedang berpindah dari RLS ke QUERY (arah desain, belum tuntas).** Syarat `deleted_at IS NULL` pada **USING `vendors_select`** **direncanakan dicabut**, sebab ia membuat **soft delete menolak dirinya sendiri** (baris yang baru terisi `deleted_at` jadi tak terlihat policy → **error 42501**). Konsekuensinya **setiap query pembaca `vendors` WAJIB menyaring `deleted_at IS NULL` sendiri** — itulah alasan filter di dropdown GRN dipasang **lebih dulu**. ⚠️ **Pencabutan RLS BELUM dijalankan** (snapshot masih memuat syarat itu) → untuk sekarang filter FE tsb masih redundan; ⚠️ **rantai sebab 42501 belum dipastikan** (syarat `deleted_at` ada di klausa **USING**, bukan `WITH CHECK`) → perlu konfirmasi Den. Detail: `03_DATA_MODEL` entri `vendors` + `PROGRESS.md` 2026-07-21.
