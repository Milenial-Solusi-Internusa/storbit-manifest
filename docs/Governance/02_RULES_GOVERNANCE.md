...
- **Skrip shell (`scripts/**`, `*.sh`) ditulis ASCII MURNI [aturan 25 Sep 2026].** `/bin/bash` di macOS = **3.2.57**, dan **di bawah locale UTF-8 ia menelan byte non-ASCII yang MENEMPEL langsung sesudah `$var` menjadi bagian NAMA variabel** — `echo "mode $mode…"` (elipsis U+2026 nempel di `$mode`) gagal **`unbound variable`** di bawah `set -u`. ⚠️ **Repro yang menentukan: GAGAL di `LC_ALL=en_US.UTF-8`, LOLOS di `LC_ALL=C`** — karena itu **`bash -n` dan uji di shell ber-`LC_CTYPE=C` TIDAK menangkapnya**, dan skrip bisa lulus semua pemeriksaan lalu mati di mesin Den. Aturannya: **jangan pernah menaruh karakter non-ASCII menempel pada variabel shell**; kalau perlu penanda non-ASCII (mis. marker yang dicocokkan `grep`), **bangun lewat `printf` oktal**, jangan ditulis literal. Berlaku juga untuk skrip di folder gitignored (`scripts/qa/out/`). Jejak: `run-night.sh`, `PROGRESS.md` 2026-09-25 butir 6.
...
| Skrip shell ASCII murni; penanda non-ASCII dibangun lewat `printf` oktal | Menaruh karakter non-ASCII **menempel** pada `$var` (bash 3.2 macOS menelannya jadi bagian nama variabel — gagal di locale UTF-8, **lolos** di `LC_ALL=C`, jadi `bash -n` tak menangkapnya) |
...
**Per task (urutan wajib):** Inspect (branch, git status, file terkait) → Plan (scope kecil) → Edit (hanya file yg perlu) → Verify (`npm run build`, lint net-zero — sejak 21 Sep 2026 lewat `node scripts/qa/lint-baseline.mjs`; bila menyentuh menu/route juga `node scripts/qa/check-menu-paths.mjs`; keduanya = isi CI. **Bila menyentuh routing/gate navigasi (sejak G1 22 Sep 2026):** jalankan sweep `scripts/qa/menu-sweep.mjs` 3 mode terhadap build yang diukur + DB staging dan bandingkan tiap mode ke baseline mode-nya sendiri — **`menu` ↔ `baseline/menu-sweep`, `restore` ↔ `baseline/menu-sweep-restore`, `path` ↔ `baseline/menu-sweep-path`** (ketiganya = hasil build G1 sejak 22 Sep 2026, keputusan Den; `--ignore` tidak wajib lagi — hanya bila sengaja mengecualikan field); perbedaan yang diharapkan saat baseline diganti wajib disebut eksplisit di `PROGRESS.md`, selain itu = regresi — aturan lengkap `scripts/qa/README.md`. **Bila menambah/memindahkan rute DETAIL ber-`:id` (sejak G2 22 Sep 2026):** sweep tidak cukup — ia hanya membuka tujuan MENU, cold load; jalankan juga `node scripts/qa/detail-routes.mjs --suite <modul>` yang menguji checklist butir 2 (refresh di detail → konteks bertahan) & butir 4 (deep-link id tak sah → keadaan wajar, bukan layar putih) . **Bila perubahan MEMINDAHKAN banyak halaman sekaligus (sejak kerangka menu Bagian 1, 24 Sep 2026):** pembandingan mentah ke baseline akan menghasilkan ratusan baris ✖ yang sebagian besar diharapkan — pakai **`node scripts/qa/sweep-classify.mjs <dir-baseline> <dir-hasil> <label> [mode]`** yang memisah **disengaja** (tujuan baru / bentuk alamat / sudah dijelaskan) dari **regresi** (status akses · `surfaceHash`/`structHash` · `headings` · tujuan baseline hilang). ⭐ **Perubahan yang dijelaskan DICETAK beserta alasannya, bukan disembunyikan** — yang berbahaya bukan perbedaannya, melainkan perbedaan yang lewat tanpa ada yang bisa menjelaskannya; kalau sebuah baris `DIJELASKAN` berhenti muncul, **itu juga kabar**. Entri penjelas dibatasi seketat mungkin (`hanyaAkun`/`hanyaMode`) supaya tidak diam-diam menelan kasus lain yang kebetulan ber-id sama. Syarat sah run tidak berubah: **gagal-muat 0** dan **cakupan akun penuh**+ klik baris/tombol kembali/Back-Forward) → Summarize (Summary / Files changed / Verification / Risk / Not changed / Next step).
...
**Asersi QA tidak boleh lolos karena prasyaratnya tak pernah terpenuhi [aturan 23 Sep 2026].** Dua kali dalam dua hari sebuah alat uji melaporkan keberhasilan padahal yang diukur bukan yang dimaksud: `menu-sweep.mjs` mencetak "✔ identik dengan baseline" saat **nol** akun berhasil login (22 Sep), dan uji butir 5 melaporkan "3/4 lolos" saat halaman yang dibuka ternyata **pentalan ke Command Center**, bukan halaman yang dituju (23 Sep). Aturannya: **asersi identitas menyebut yang spesifik** (heading/jumlah/akun), dan **kalau asersi prasyarat gagal, asersi turunannya jangan dijalankan** — berhenti dengan exit gagal, lebih baik nol angka daripada angka yang terbaca seperti keberhasilan. Resep + jejaknya: `scripts/qa/README.md` §Checklist (sengaja dicatat sebagai pelajaran proses, **bukan** TD).

**Aturan yang sama berlaku untuk uji SQL/migrasi, bukan cuma harness QA [perluasan 25 Sep 2026].** Dalam satu hari **tiga** blok verifikasi migrasi terbukti hijau tanpa menguji apa pun: **(a)** uji "index unik mengikat" menyisipkan baris duplikat dengan hanya 5 kolom, sehingga **gagal lebih dulu** di `company_id`/`product_id` yang `NOT NULL` — cek uniknya **tak pernah tersentuh**, dan error `NOT NULL` itu terbaca sebagai bukti index bekerja; **(b)** uji "PUBLIC sudah dicabut" memakai `LIKE '%=X/%'`, pola yang **juga** cocok pada `postgres=X/postgres` sehingga **selalu true**; **(c)** sebuah verifikasi bernilai 5 yang **bukan** kegagalan, dan baru bisa dibedakan setelah asal kelima barisnya diselidiki. Aturan turunannya: **(1)** tiap uji ber-harapan "ditolak" wajib punya **pembanding yang diterima** — tanpa itu, "ditolak" bisa berarti guardnya bekerja **atau** berarti tak ada yang pernah bisa lolos; **(2)** pengujian constraint wajib menyisipkan baris yang **lolos semua syarat lain** (salin seluruh kolom dari baris yang sudah ada, ganti hanya yang perlu) dan memakai cabang yang **membedakan kode error** (`WHEN OTHERS` yang memisahkan "ditolak karena duplikat" dari "ditolak karena hal lain"); **(3)** uji ber-pola string wajib dijalankan sekali pada kasus yang **seharusnya gagal**, untuk membuktikan ia bisa gagal. Detail + kasusnya: `03_DATA_MODEL.md` gotcha **#40** · `PROGRESS.md` 2026-09-25.
