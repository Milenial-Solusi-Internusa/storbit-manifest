...
- **Import lokal** (relative path), bukan alias. Lucide untuk SEMUA ikon. **[21 Sep 2026, Batch FS Fase 2.5 G0]** Alias **`@/` → `src/`** kini ADA (`vite.config.js` regex `^@/` + `jsconfig.json`; Fase 0 §7) tapi **hanya untuk `src/routes/*` dulu** — file lain tetap impor relatif sampai giliran Batch FS-nya; hari ini nol pemakai (`src/routes/menu-paths.js` tanpa impor). Jangan mulai mengganti impor relatif ke `@/` sepihak.
