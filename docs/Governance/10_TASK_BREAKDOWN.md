...
- [ ] **H7. Level 2 — Kunci Harga Edit Item SP (🟡 SEBAGIAN DIEKSEKUSI — belum tuntas).** Hasil sesi perancangan; **1 sub-item UI sudah dikerjakan** (harga read-only di EditItemModal — branch `fix/lock-price-edit-item`, **belum merge, belum tes runtime**). Backfill/RLS/dropdown kategori/migrasi **belum jalan** (kecuali 1 ALTER TABLE, lihat status). Direkam agar konteks tak hilang antar-sesi.
  ...
  - **STATUS EKSEKUSI:**
    ...
    - ✅ Kolom **`price_category` SUDAH ditambah ke `sp_items`** (ALTER manual). Domain nilai (`default`/`semester`/`legacy` …) **berbeda** dari CHECK `sp_order_items.price_category` (`semester/tahunan/project`) → **perlu konfirmasi user**, jangan diputuskan sepihak. ⚠️ `schema_snapshot.sql` **belum memuatnya** (stale — lihat TD-72).
  ...
  - **BELUM dikerjakan (sesi berikutnya):**
    ...
    - [ ] **Refresh `schema_snapshot.sql` (pg_dump manual user) — MASIH PENDING.** `price_category` **dan** `review_status` belum termuat di snapshot (terverifikasi 2026-07-16). Rekam-migrasi sudah beres (lihat STATUS EKSEKUSI); sisa utang TD-72 tinggal refresh snapshot ini.
