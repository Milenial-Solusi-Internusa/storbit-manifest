...
| TD-05 | HIGH | **Tidak ada audit logging.** `AGENTS.md`/security baseline wajibkan 19 event audit; saat ini **0 call** `logAudit()` di kode, tabel `audit_logs` belum diimplementasi. | seluruh app | OPEN | Implement `audit_logs` + `logAudit()` helper. Konsumen menunggu: `AuditLogPage` kini login-only (**TD-37**) → ganti sumber ke `audit_logs` setelah jadi. |
...
| TD-37 | LOW | **AuditLog baru tampil login events.** `AuditLogPage` fetch real dari `user_login_logs` (login saja); filter modul/aksi (create/update/delete/approve/export) tampil kosong karena belum ada sumbernya. | `src/pages/foundation/admin-settings/AuditLogPage.jsx` | OPEN | Tunggu tabel `audit_logs` + `logAudit()` (lihat **TD-05**); setelah jadi, ganti sumber AuditLog dari `user_login_logs` → `audit_logs` (atau union). |
