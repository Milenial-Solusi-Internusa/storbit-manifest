...
| | **"Tandai Kalah" per-inquiry** (`status='LOST'` + `lost_reason` wajib) | ✅ | 22 Jul, `DealDetailPage`; allowlist status belum-terminal (fail-closed); `accounts` nol sentuhan; izin dari **RLS `inquiries_update`** (sales hanya inquiry buatannya); **berjejak di `audit_logs`** (`UPDATE_INQUIRY`, notes memuat status lama + alasan). ⚠️ **belum tes runtime** |
