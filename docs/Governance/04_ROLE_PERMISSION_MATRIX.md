...
| CRM | ↳ **Cetak PRF** (aksi di Detail Inquiry & Detail Account tab Riwayat) | DealDetailPage (`PrfListCard canCreate`) · CustomerDetailPage (`canCreatePRF`) | role[sales,gm_bd,**super_admin**] — **`super_admin` ditambah 27 Jul 2026** (`feat/prf-audit`, dua array identik `['sales','gm_bd']`→`['sales','gm_bd','super_admin']`, `DealDetailPage.jsx:483`+`CustomerDetailPage.jsx:724`) | LIVE — **dikonfirmasi kelewatan, bukan keputusan sadar**: `super_admin` sebelumnya tak bisa memicu pembuatan PRF dari kedua entry point resmi meski RLS `prf_insert` **dan** gate menu registry `'prf'` (`App.jsx:695`, 7 role) sama-sama sudah mengizinkannya. Gate lain (RLS, `canRenderPage`, registry menu) **tak disentuh**. Lihat `08_TECH_DEBT` TD-105 (array yang sama = contoh andalan TD itu; penambahan ini **widening sempit**, BUKAN penyelesaian akar TD-105 — user multi-role spt Endang Kumolo Ratih tetap kehilangan hak `sales`-nya). ⚠️ Belum tes runtime. |
