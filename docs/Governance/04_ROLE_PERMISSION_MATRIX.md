...
| PROCUREMENT | Vendor Management › Vendor List (Master Vendor) | VendorListPage | role[procurement,manager,supervisor,gm,gm_bd,ceo,admin,super_admin] — **cermin persis RLS `vendors_*`** = `has_role('procurement')` + ketujuh kode `is_manager_or_above()` (`schema_snapshot.sql:956`); **`sales` sengaja dikecualikan**. Gate lewat `role[]` array, BUKAN menuKey (`MENU_KEY_MAP` tak punya entri `proc-vendor-list`) | LIVE (21 Jul 2026 — CRUD master data, FE-only, NOL perubahan DB/RLS). ✅ **Tes runtime SEBAGIAN (21 Jul 2026, login `procurement` asli = SCM Master, bukan super_admin):** menu muncul · leaf aktif (tanpa `soon`) · halaman terbuka tanpa error konsol · **list company-scoped benar** (user MSI melihat 1 vendor MSI, bukan 4 seed SOA) → gate positif-case + jalur SELECT/INSERT RLS terbukti; negative-case `sales` sudah terbukti di sesi sebelumnya (menu tidak muncul). ⚠️ **Belum teruji: edit tersimpan & "Nonaktifkan" (soft delete).** ⚠️ `role` di `canSeeMenuItem` = **role primer** (`erpRole`) → user `sales`+`procurement` / `finance`+`procurement` **tak melihat menu** meski RLS mengizinkan (`has_role()` menguji semua role) → gate FE **lebih ketat dari RLS**, lihat **TD-105**. `supervisor` dalam daftar = ikut cermin-RLS tapi **mati** (tak ada di tabel `roles`) → **TD-106** |
