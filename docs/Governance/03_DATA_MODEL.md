...
>
> **Diperbarui 2026-07-10 (audit CRM E2E) — koreksi CRM:** `accounts` (pipeline = tabel ini, tak ada `deals`; `estimated_value`; kolom Lead Pool/aging), mesin status `quotations` (DRAFT→SUBMITTED→SENT), `rate_sheets` RLS bocor, `notification_rules` yatim, trigger `sync_last_activity`; gotcha #10-12 (snapshot stale lagi + data historis CRM + pooler host). Sebelumnya (8 Jul): FASE 0-3 mesin status SP. Hitungan tabel/RPC kanonik: `supabase/schema_snapshot.sql`.
...
- **`sp_items`** — item SP generasi lama; **MASIH DIBACA** reader fulfillment (list/detail, `shipped_qty`). Create SP = **dual-write** (`create_sp_order_dual` tulis `sp_items` LAMA **dan** `sp_orders`/`sp_order_items` baru). FK `sp_items_customer_id_fkey` → `accounts`. RLS ⚠️ `USING(true)`. **+kolom `price_category`** [BARU — ALTER dijalankan manual, ⚠️ **belum di `schema_snapshot.sql`** & **belum direkam migrasi**] untuk rencana "Kunci Harga Edit Item SP" (§Task H7 / `10_TASK_BREAKDOWN.md`). ⚠️ **Domain nilai belum baku** — dipakai `default`/`semester`/`legacy` (mengikuti kategori Master Product + penanda SP lama), **BEDA** dari CHECK `sp_order_items.price_category` (`semester/tahunan/project` saja). Perbedaan domain ini **perlu dikonfirmasi user** (belum ada CHECK constraint di `sp_items.price_category`).
