...
> ⛔ **STATUS KESELURUHAN (diperbarui 20 Sep 2026 malam): KEBIJAKAN SUDAH DIPUTUSKAN; Batch DS 1 SELESAI — kit tunggal ADA sebagai KODE MATI di `src/kit/` (12 file) + `src/lib/entities.js`, BELUM DIPAKAI SATU HALAMAN PUN; tampilan aplikasi BELUM BERUBAH.** Bagian **A** di bawah memuat keputusan Den 19 Sep 2026: palet aplikasi **sage** + **tiga font resmi** + arah **satu kit tunggal**, plus **rencana Batch DS 1–7 (A.4) yang disetujui 20 Sep 2026 — Batch DS 1 selesai (kode mati, menunggu review Den), Batch DS 2–7 belum mulai**. **Yang sudah ada di kode = kit-nya saja** (`tokens.js` sage + komponen + etalase, nol pemakai; `kit.css` belum diimpor `main.jsx`); penggantian warna/font di halaman, pelepasan kit lama, dan dua temuan TD-269/TD-270 **BELUM dikerjakan** (tutup resmi Batch DS 6). Bagian §0–§17 memotret kode `main` @ `660168f` apa adanya (palet navy/oranye lama, tiga keluarga token, font legacy) dan **tetap benar sebagai gambaran halaman yang dilihat user hari ini** — satu-satunya tambahan sejak itu = `src/kit/` + `src/lib/entities.js` (baris baru di §0 dan §1). Siapa pun (termasuk sesi Claude Code lain) yang membaca dokumen ini: **jangan mengira halaman sudah memakai sage, dan jangan mengimpor `src/kit` ke halaman mana pun di luar giliran Batch DS-nya (A.4.3)** — #61 tetap berlaku sampai Den memutuskan lain.
>
...
**Status (20 Sep 2026 malam): Batch DS 1 SELESAI — `tokens.js` sage + komponen tunggal + etalase ADA di `src/kit/` (12 file) dan `src/lib/entities.js`, sebagai KODE MATI (nol pemakai; `kit.css` belum diimpor `main.jsx`; etalase tidak di-route). BUKAN klaim penyatuan selesai:** belum satu halaman pun dimigrasi; `#144682`/`#1B4D8A`/`#E85A1E`/`#E8703D` masih hidup di kode halaman (sebaran di §3.1 — `#144682`/`#E85A1E` yang ada di `src/kit/tokens.js` = kelas STATUS `TONE`/`DEAL_STATUS.WON`, keputusan #3, bukan identitas); kit lama tetap dipakai 22 halaman AdminKit + 3 halaman v3. **Rencana Batch DS 1–7 (A.4):** Batch DS 1 ✅ menunggu review Den; Batch DS 2–7 **belum dimulai**, satu Batch DS per giliran atas instruksi Den. Dua temuan kode: **TD-269** (dark green `#2F6B3F` di 5 file + emoji) dan **TD-270** (`SaveButton` AdminKit tidak menunggu Promise) — **keduanya TETAP OPEN**: kit baru hanya membangun jalannya (`src/kit/Button.jsx` `SaveButton` ber-`await`; `src/kit/Data.jsx:125` `<Check/>` Lucide, bukan `'✓'`), file yang bermasalah belum disentuh; tutup resmi di Batch DS 6.
...
| **Palet STATUS/DATA — kelas terpisah (#60 b)** | `SEMANTIC` ok/warn/danger/info/neutral (trio fg/bg/bd; nilai dari warm-beige yang sudah dipakai 40+ file, `neutral` diturunkan ulang dari sage) · `TONE` slate/orange/navy/brick (v3, apa adanya) · `DEAL_STATUS` 7 status (`STATUS_META` yang sudah lolos AA, WON navy) · helper `toneOf`/`closedFrom`/`isClosedStage`. ⚠️ **Terukur 20 Sep malam atas nilai final (`tokens.js:90-93/:105-106`): `SEMANTIC.ok` 4,30 · `warn` 3,99 · `TONE.slate` 4,23 · `TONE.orange` 3,23 — di bawah AA 4,5:1 untuk teks badge, dibawa apa adanya (keputusan #3) → Keputusan Terbuka #62** |
...
3. **Palet status/data** = trio semantik warm-beige yang sudah ada (ok/warn/danger/info), `TONE` v3, dan `DEAL_STATUS` 7 status **termasuk WON navy `#144682` sebagai warna STATUS** (bukan identitas); warna slice donut/funnel/chart = kelas status, tidak disentuh.
...
| **Kit tunggal `src/kit/` + `src/lib/entities.js`** (lahir Batch DS 1, 20 Sep 2026 malam) | ⛔ **JANGAN dipakai halaman apa pun dulu — KODE MATI, nol pemakai.** Halaman masuk kit hanya lewat giliran Batch DS-nya (A.4.3: Batch DS 2 = 22 file AdminKit + `main.jsx` impor `kit.css`; Batch DS 3 = CRM v3; dst.), masing-masing atas instruksi Den. | #61 tetap berlaku: "kit ada" ≠ "instruksi implementasi turun" untuk semua halaman. Mengimpor `src/kit` sepihak = migrasi di luar rencana, dua kit di satu halaman, dan screenshot "sebelum/sesudah" per Batch DS jadi tak bermakna. Isi kit: A.4.2 + §1 (baris `src/kit/`). |
...
| **`src/kit/`** (12 file, 2.335 baris): `tokens.js` (166) · `kit.css` (56) · `Button.jsx` (245) · `Form.jsx` (372) · `Layout.jsx` (268) · `Data.jsx` (431) · `Overlay.jsx` (154) · `Upload.jsx` (108) · `Icon.jsx` (64) · `useToast.jsx` (56) · `index.js` (31) · `__demo/KitGallery.jsx` (384) | 20 Sep 2026 (Batch DS 1) | **Kit tunggal ber-palet sage + tiga font** (A.4.2): 9 hex A.1 + turunan berumus; palet STATUS/DATA kelas terpisah (`SEMANTIC`/`TONE`/`DEAL_STATUS`); **31 export** (komponen + `Icon` + `useToast`) + **5 alias** nama lama (`PrimaryBtn`/`OutlineBtn`/`KitSelect`/`Skel`/`Tabs`, dicabut Batch DS 7); `FormSheet`/`Notebook`/`StatusBar`/`ListView` = **salinan v3** berwarna sage (file v3 asli tak disentuh, dihapus Batch DS 3); `Icon` registri 66 nama identik AdminKit; `kit.css` prefiks `nk-` (≠ `ak-`); `SaveButton` menunggu Promise (arah TD-270) | **NOL — KODE MATI.** Tidak diimpor satu file pun di luar `src/kit/` (satu-satunya `from '../kit'` lain = `crm/v3/__demo/CrmV3DemoPage.jsx:24` → `crm/v3/kit.jsx`); `kit.css` belum diimpor `main.jsx`; etalase tidak di-route. Konsumen pertama = Batch DS 2. Tampilan aplikasi belum berubah. |
...
// di luar giliran Batch DS-nya (A.4.3, #61). Bentuk impor yang akan dipakai Batch DS 2+ (path relatif —
