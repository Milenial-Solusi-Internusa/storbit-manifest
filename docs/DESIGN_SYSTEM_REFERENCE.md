...
**Status: ⛔ BELUM DIKERJAKAN — murni arah tujuan yang sudah diputuskan, BUKAN klaim sudah selesai.** Belum ada `tokens.js` sage, belum ada komponen tunggal, belum ada satu halaman pun yang dimigrasi, `#144682`/`#1B4D8A`/`#E85A1E`/`#E8703D` masih hidup di kode (sebaran di §3.1). **Rencana Batch 1–7 sudah disetujui Den 20 Sep 2026 (A.4)** — eksekusinya **belum dimulai**, dijalankan Batch demi Batch atas instruksi Den, satu Batch per giliran. Dua temuan kode yang ikut menunggu batch ini: **TD-269** (dark green `#2F6B3F` di 5 file) dan **TD-270** (`SaveButton` tidak menunggu Promise) — keduanya OPEN di `08_TECH_DEBT.md`, belum diperbaiki.
...
### A.4 Rencana Penyatuan Kit — Batch 1 s/d 7 (plan disetujui Den 20 Sep 2026) — ⛔ BELUM DIKERJAKAN, belum ada Batch yang mulai

> Istilah **"Batch"** dipakai di sini (bukan "Wave") supaya tidak bentrok dengan istilah Wave di Grand Design roadmap terpisah. Status per 20 Sep 2026: **nol Batch dimulai, nol file kode berubah** — bagian ini adalah rencana yang sudah disetujui, bukan laporan kemajuan. Kolom "Status" di tabel Batch diperbarui saat tiap Batch selesai di-review. Baseline sebelum Batch 1: `npm run build` clean **2.636 modul** · `npm run lint` **160 (139 error / 21 warning)**.
...
**A.4.4 Protokol verifikasi (tiap Batch)**

| # | Gate |
|---|---|
| 1 | `npm run build` clean (jumlah modul dicatat) · `npm run lint` ≤ 160 (139/21) dan `npx eslint <file Batch>` tidak menambah error |
| 2 | `git diff --stat` **persis** daftar file Batch; gate keras setiap Batch: `git diff --stat -- src/modules/logistics/print* src/modules/logistics/*PDF.jsx src/modules/crm/QuotationPDF.jsx` **kosong**, dan blok `.kpi`/Oswald di `CRMDashboardPage` nol diff |
| 3 | Grep gate hex lama per Batch (angka turun; nol hex baru di luar `tokens.js`) |
| 4 | Screenshot **sebelum/sesudah**, desktop 1280 + mobile 375, halaman kunci: Home/App Launcher · CRM Dashboard · Inquiry List · Detail Deal · Pipeline · Customer List · Quotation Form · Storbit Dashboard · Detail SP · SP List · Picking/Surat Jalan · AdminHub · Companies · User Access · Entity Settings. "Sebelum" diambil sekali di Batch 1; disimpan di luar repo dan dikirim ke Den per Batch |
| 5 | Batch berlabel **YA**: branch → Vercel preview + DB staging → smoke test oleh Den (login) → baru merge `main` |
| 6 | Doc-keeper di akhir tiap Batch (PROGRESS + CLAUDE Recent + TD + kolom Status tabel A.4.3) |

**A.4.5 Risiko yang sudah diketahui**

| Risiko | Penanganan |
|---|---|
| Batch 3, 4c, 4e, 4g, 5 = halaman trafik tinggi (CRM sales harian, shell semua halaman, Storbit ops) | wajib preview + staging sebelum `main` |
...
| `SaveButton` berubah perilaku (menunggu server) — pemanggil yang `onSave`-nya tidak mengembalikan Promise akan tetap "sukses instan" | tiap pemanggil dicek di Batch 2 |
| Kartu ≈ halaman (1,05:1) → tampilan lebih "rata" | konsekuensi palet, bukan bug; kalau screenshot Batch 2 terasa kurang berhierarki, penyesuaian (mis. bayangan tipis turunan) = keputusan Den, bukan mengubah 7 hex |
...
4. **Batch 4f masuk scope sekarang** (50 file hex + 194 kelas Tailwind) sebagai giliran terakhir Batch 4 — tanpa ini kit tunggal tidak tunggal.
5. **TD-270 dibangun benar sejak Batch 1** (`SaveButton` async); Batch 6 tinggal verifikasi & tutup TD.
6. **TD-70 (font legacy) + `index.css :root` + `tailwind.config.js` masuk Batch 4e** bersama `App.jsx`; Oswald tetap.
...
9. **`Chatter` v3 (0 pemakai) dan `CrmV3DemoPage` dihapus** di Batch 3; `InquiryChatter.jsx` tetap yang hidup.
10. **Kosakata CRM keluar dari kit** (`crm/dealStatus.js`); **`ENTITIES` tunggal** di `src/lib/entities.js`.
11. **Alur kerja**: branch per Batch (`feat/kit-b1-tokens` …), satu commit per Batch, merge ke `main` setelah review Den; push hanya atas instruksi; doc-keeper tiap akhir Batch.
12. **Screenshot** disimpan di luar repo dan dikirim per Batch, tidak di-commit.
13. **Env staging** sudah dibereskan 20 Sep 2026 (`.env.local`/`.env.staging` → `nexus-staging`), prasyarat Batch bertanda YA terpenuhi.
