...
> ⛔ **STATUS KESELURUHAN (diperbarui 20 Sep 2026 malam): KEBIJAKAN SUDAH DIPUTUSKAN; Batch DS 1 SELESAI — kit tunggal ADA sebagai KODE MATI di `src/kit/` (12 file) + `src/lib/entities.js`, BELUM DIPAKAI SATU HALAMAN PUN; tampilan aplikasi BELUM BERUBAH.** Bagian **A** di bawah memuat keputusan Den 19 Sep 2026: palet aplikasi **sage** + **tiga font resmi** + arah **satu kit tunggal**, plus **rencana Batch DS 1–7 (A.4) yang disetujui 20 Sep 2026 — Batch DS 1 selesai (kode mati) dan review-nya DITUTUP Den (sesi keenam, 20 Sep 2026: #62 dijawab — empat fg kelas STATUS di `tokens.js` digeser minimal ke AA; #61 final), Batch DS 2–7 belum mulai**. **Yang sudah ada di kode = kit-nya saja** (`tokens.js` sage + komponen + etalase, nol pemakai; `kit.css` belum diimpor `main.jsx`); penggantian warna/font di halaman, pelepasan kit lama, dan dua temuan TD-269/TD-270 **BELUM dikerjakan** (tutup resmi Batch DS 6). Bagian §0–§17 memotret kode `main` @ `660168f` apa adanya (palet navy/oranye lama, tiga keluarga token, font legacy) dan **tetap benar sebagai gambaran halaman yang dilihat user hari ini** — satu-satunya tambahan sejak itu = `src/kit/` + `src/lib/entities.js` (baris baru di §0 dan §1). Siapa pun (termasuk sesi Claude Code lain) yang membaca dokumen ini: **jangan mengira halaman sudah memakai sage, dan jangan mengimpor `src/kit` ke halaman mana pun di luar giliran Batch DS-nya (A.4.3)** — #61 **FINAL** (dikonfirmasi Den sesi keenam): berhenti per halaman saat Batch DS-nya selesai, sepenuhnya setelah Batch DS 7.
>
...
## A. Kebijakan resmi (keputusan Den 19 Sep 2026) & arah penyatuan kit — Batch DS 1 ✅ selesai 20 Sep 2026 (kit = KODE MATI, nol pemakai); Batch DS 2–7 ⛔ belum mulai
...
**Status (20 Sep 2026 malam): Batch DS 1 SELESAI — `tokens.js` sage + komponen tunggal + etalase ADA di `src/kit/` (12 file) dan `src/lib/entities.js`, sebagai KODE MATI (nol pemakai; `kit.css` belum diimpor `main.jsx`; etalase tidak di-route). BUKAN klaim penyatuan selesai:** belum satu halaman pun dimigrasi; `#144682`/`#1B4D8A`/`#E85A1E`/`#E8703D` masih hidup di kode halaman (sebaran di §3.1 — `#144682` yang ada di `src/kit/tokens.js` = kelas STATUS `TONE.navy`/`DEAL_STATUS.WON`, keputusan #3, bukan identitas; `#E85A1E` **sudah nol di kit** sejak #62 — `TONE.orange` fg kini `#BE4A19`); kit lama tetap dipakai 22 halaman AdminKit + 3 halaman v3. **Rencana Batch DS 1–7 (A.4):** Batch DS 1 ✅ **review DITUTUP Den 20 Sep 2026 (sesi keenam)** — kode + screenshot diterima, hasil = #62 (4 fg STATUS digeser ke AA, kontras diukur ulang 0 gagal teks/status) + #61 final, nol permintaan lain; Batch DS 2–7 **belum dimulai**, satu Batch DS per giliran atas instruksi Den. Dua temuan kode: **TD-269** (dark green `#2F6B3F` di 5 file + emoji) dan **TD-270** (`SaveButton` AdminKit tidak menunggu Promise) — **keduanya TETAP OPEN**: kit baru hanya membangun jalannya (`src/kit/Button.jsx` `SaveButton` ber-`await`; `src/kit/Data.jsx:125` `<Check/>` Lucide, bukan `'✓'`), file yang bermasalah belum disentuh; tutup resmi di Batch DS 6.
...
### A.4 Rencana Penyatuan Kit — Batch DS 1 s/d 7 (plan disetujui Den 20 Sep 2026) — Batch DS 1 ✅ selesai 20 Sep 2026 (kit lahir sebagai kode mati); Batch DS 2–7 ⛔ belum mulai

> Istilah **"Batch DS"** (DS = design system) dipakai untuk seluruh rencana ini — **bukan "Wave"** (istilah itu milik Grand Design roadmap terpisah) dan **bukan "Batch" polos** (yang sudah dipakai Batch CRM v3 B1–B6/3C dan Batch 1–3 migrasi TD-14 Agustus 2026). Status per 20 Sep 2026 (malam): **Batch DS 1 selesai — `src/kit/` (12 file) + `src/lib/entities.js` lahir sebagai KODE MATI, nol file lama disentuh, nol halaman dimigrasi; Batch DS 2–7 belum dimulai** — kolom "Status" tabel A.4.3 adalah satu-satunya laporan kemajuan di bagian ini, sisanya rencana yang sudah disetujui. Kolom "Status" di tabel Batch DS diperbarui saat tiap Batch DS selesai di-review. Baseline sebelum Batch DS 1: `npm run build` clean **2.636 modul** · `npm run lint` **160 (139 error / 21 warning)**.

**A.4.1 Cakupan nyata (diukur 20 Sep 2026, bukan angka kebijakan)**

| Kelompok | Angka kebijakan | Terukur |
|---|---|---|
| Konsumen AdminKit | 22 file | **22** (11 `admin-settings/` + `AdminHub.jsx` + 10 `modules/admin/pages/`), semuanya masih di-route lewat `AdminHub` |
...
| `src/lib/entities.js` | `ENTITIES` tunggal (code + UUID + nama legal) — menggantikan dua versi beda bentuk di dua kit |
...
| `SaveButton` berubah perilaku (menunggu server) — pemanggil yang `onSave`-nya tidak mengembalikan Promise akan tetap "sukses instan" | tiap pemanggil dicek di Batch DS 2 |
...
| **Kit tunggal `src/kit/` + `src/lib/entities.js`** (lahir Batch DS 1, 20 Sep 2026 malam) | ⛔ **JANGAN dipakai halaman apa pun dulu — KODE MATI, nol pemakai.** Halaman masuk kit hanya lewat giliran Batch DS-nya (A.4.3: Batch DS 2 = 22 file AdminKit + `main.jsx` impor `kit.css`; Batch DS 3 = CRM v3; dst.), masing-masing atas instruksi Den. | #61 tetap berlaku — **FINAL, dikonfirmasi Den 20 Sep 2026 (sesi keenam)**: "kit ada" ≠ "instruksi implementasi turun" untuk semua halaman; berhenti per halaman saat Batch DS-nya selesai. Mengimpor `src/kit` sepihak = migrasi di luar rencana, dua kit di satu halaman, dan screenshot "sebelum/sesudah" per Batch DS jadi tak bermakna. Isi kit: A.4.2 + §1 (baris `src/kit/`). |
...
| **`src/kit/`** (12 file, ~~2.335~~ **2.345** baris — `wc -l` sesudah #62): `tokens.js` (~~166~~ **175**) · `kit.css` (56) · `Button.jsx` (245) · `Form.jsx` (372) · `Layout.jsx` (268) · `Data.jsx` (431) · `Overlay.jsx` (154) · `Upload.jsx` (108) · `Icon.jsx` (64) · `useToast.jsx` (56) · `index.js` (~~31~~ **32**) · `__demo/KitGallery.jsx` (384) | 20 Sep 2026 (Batch DS 1) | **Kit tunggal ber-palet sage + tiga font** (A.4.2): 9 hex A.1 + turunan berumus; palet STATUS/DATA kelas terpisah (`SEMANTIC`/`TONE`/`DEAL_STATUS`; sejak #62 [20 Sep, sesi keenam] empat fg `ok`/`warn`/`slate`/`orange` digeser minimal ke AA 4,5:1 — seluruh pasangan teks/status kit lolos AA); **31 export** (komponen + `Icon` + `useToast`) + **5 alias** nama lama (`PrimaryBtn`/`OutlineBtn`/`KitSelect`/`Skel`/`Tabs`, dicabut Batch DS 7); `FormSheet`/`Notebook`/`StatusBar`/`ListView` = **salinan v3** berwarna sage (file v3 asli tak disentuh, dihapus Batch DS 3); `Icon` registri 66 nama identik AdminKit; `kit.css` prefiks `nk-` (≠ `ak-`); `SaveButton` menunggu Promise (arah TD-270) | **NOL — KODE MATI.** Tidak diimpor satu file pun di luar `src/kit/` (satu-satunya `from '../kit'` lain = `crm/v3/__demo/CrmV3DemoPage.jsx:24` → `crm/v3/kit.jsx`); `kit.css` belum diimpor `main.jsx`; etalase tidak di-route. Konsumen pertama = Batch DS 2. Tampilan aplikasi belum berubah. |
| `src/lib/entities.js` (39 baris) | 20 Sep 2026 (Batch DS 1) | `ENTITIES` tunggal `{ code, id: UUID companies.id, name legal }` (SOA "PT Stuja Orbit Abadi") + `ENTITY_CODES` + `ENTITY_BY_CODE`/`ENTITY_BY_ID` (frozen) + `entityByCode`/`entityById`/`entityCodeOf` | `src/kit/Form.jsx` (`EntitySwitcher`, banding `e.code`) saja. Dua versi lama tetap hidup s/d Batch DS 7 (`admin-settings/tokens.js:29` id = kode tanpa UUID · `crm/v3/tokens.js:147` id = UUID); versi lokal lain yang juga harus dilebur: `PositionsPage.jsx:46` (UUID, nama tanpa "PT" — Batch DS 2) · **`CRMReportPage.jsx:61`** `["MSI","JCI","SOA"]` (array kode — *temuan doc-keeper 20 Sep malam, belum disebut rencana; kandidat Batch DS 4c*) |
...
// Kit tunggal (src/kit/) — ⛔ KODE MATI sejak Batch DS 1 (20 Sep 2026): JANGAN diimpor halaman mana pun
// di luar giliran Batch DS-nya (A.4.3, #61 — FINAL, dikonfirmasi Den 20 Sep 2026). Bentuk impor yang akan dipakai Batch DS 2+ (path relatif —
...
// `kit.css` diimpor SEKALI di src/main.jsx (Batch DS 2) — bukan per halaman, bukan <KitStyles/>.
// Bersama
