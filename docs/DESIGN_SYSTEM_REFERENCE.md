...
**Status: ⛔ BELUM DIKERJAKAN — murni arah tujuan yang sudah diputuskan, BUKAN klaim sudah selesai.** Belum ada `tokens.js` sage, belum ada komponen tunggal, belum ada satu halaman pun yang dimigrasi, `#144682`/`#1B4D8A`/`#E85A1E`/`#E8703D` masih hidup di kode (sebaran di §3.1). **Rencana Batch DS 1–7 sudah disetujui Den 20 Sep 2026 (A.4)** — eksekusinya **belum dimulai**, dijalankan Batch DS demi Batch DS atas instruksi Den, satu Batch DS per giliran. Dua temuan kode yang ikut menunggu batch ini: **TD-269** (dark green `#2F6B3F` di 5 file) dan **TD-270** (`SaveButton` tidak menunggu Promise) — keduanya OPEN di `08_TECH_DEBT.md`, belum diperbaiki.
...
### A.4 Rencana Penyatuan Kit — Batch DS 1 s/d 7 (plan disetujui Den 20 Sep 2026) — ⛔ BELUM DIKERJAKAN, belum ada Batch DS yang mulai

> Istilah **"Batch DS"** (DS = design system) dipakai untuk seluruh rencana ini — **bukan "Wave"** (istilah itu milik Grand Design roadmap terpisah) dan **bukan "Batch" polos** (yang sudah dipakai Batch CRM v3 B1–B6/3C dan Batch 1–3 migrasi TD-14 Agustus 2026). Status per 20 Sep 2026: **nol Batch DS dimulai, nol file kode berubah** — bagian ini adalah rencana yang sudah disetujui, bukan laporan kemajuan. Kolom "Status" di tabel Batch DS diperbarui saat tiap Batch DS selesai di-review. Baseline sebelum Batch DS 1: `npm run build` clean **2.636 modul** · `npm run lint` **160 (139 error / 21 warning)**.
...
| `Layout.jsx` | `PageHeader`, `SectionLabel`, `Card`, `EmptyState`, `Skeleton`, `FormSheet` (pindahan v3), `Notebook` (pindahan v3; menyerap `Tabs` AdminKit: +`icon`/+`dot`) |
| `Data.jsx` | `ListView` (pindahan v3, mode table/lanes), `Badge`, `DocNo`, `StatusBar` (pindahan v3) |
...
| `src/modules/crm/dealStatus.js` | `STATUS_LABEL`, `DEAL_STATUS_ORDER`, `LIFECYCLE_ORDER` — kosakata CRM keluar dari kit (kit harus buta sumbu) |
...
| **Palet STATUS/DATA — kelas terpisah (#60 b)** | `SEMANTIC` ok/warn/danger/info/neutral (trio fg/bg/bd; nilai dari warm-beige yang sudah dipakai 40+ file, `neutral` diturunkan ulang dari sage) · `TONE` slate/orange/navy/brick (v3, apa adanya) · `DEAL_STATUS` 7 status (`STATUS_META` yang sudah lolos AA, WON navy) · helper `toneOf`/`closedFrom`/`isClosedStage` |
...
|---|---|---|---|---|
| **1 — Kit tunggal lahir, belum dipakai** | 12 file baru `src/kit/` + `src/lib/entities.js` (+ `__demo/KitGallery.jsx`); nol file lama disentuh; `SaveButton` langsung benar (await Promise, jalur error). **Sekaligus ambil screenshot "SEBELUM" semua halaman kunci (A.4.4) selagi tampilan masih lama.** | build (+modul), lint net-zero (file baru 0 error), skrip kontras dijalankan ulang atas nilai token final, screenshot KitGallery (mount sementara, tanpa DB) | Tidak — kode mati | ⛔ belum mulai |
