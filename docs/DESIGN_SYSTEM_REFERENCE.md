...
Penyatuan **penuh** menyentuh ±110 file. Dua fakta palet yang menentukan desain kit (WCAG, dihitung): teks putih di atas aksen sage **gagal** (1,5–1,95:1) → semua pola "solid navy/oranye + teks putih" menjadi **latar aksen + teks `#33422B`** (7,1:1); kartu vs halaman **1,05:1** dan border vs kartu 1,2:1 → hierarki bertumpu pada border + bayangan turunan, dan fokus ring memakai `#5C6B52`/`#33422B` (aksen hanya 1,4:1 terhadap halaman).
...
| Kelompok | Isi |
|---|---|
| Palet identitas (9, persis A.1) | `BG #F6F8F3` · `CARD #EEF3EA` · `ACCENT #C3D9B8` / `ACCENT_HOVER #AFC9A0` · `ACCENT_2 #D7C9B0` / `ACCENT_2_HOVER #C9B896` · `LINE #D8E0D2` · `INK #33422B` · `INK_SOFT #5C6B52` |
| Turunan (dihitung dari 9 di atas, **nol hue baru**, rumus ditulis di komentar) | `INK_FAINT` (placeholder/hint), `LINE_SOFT`, `ROW_HOVER`, `HEAD_BG` (thead/header kartu), `INPUT_BG` (= `BG`, supaya input timbul di atas kartu), `DISABLED_BG`/`DISABLED_INK`, `FOCUS_RING` (rgba `INK_SOFT` .35, 3px), `BACKDROP` (rgba `INK` .45), `SHADOW_1`/`SHADOW_2` (rgba `INK`) |
...
| Kartu ≈ halaman (1,05:1) → tampilan lebih "rata" | konsekuensi palet, bukan bug; kalau screenshot Batch DS 2 terasa kurang berhierarki, penyesuaian (mis. bayangan tipis turunan) = keputusan Den, bukan mengubah 7 hex |
...
1. **Token turunan diizinkan** — hover/disabled/fokus/backdrop/bayangan/`LINE_SOFT`/`ROW_HOVER` dihitung dari 9 hex kebijakan, tanpa hue baru, rumus tercatat di `tokens.js`.
...
**Resmi sejak 19 Sep 2026 (Bagian A.1):** latar halaman `#F6F8F3` · latar kartu `#EEF3EA` · aksen utama `#C3D9B8` (hover `#AFC9A0`) · aksen kedua `#D7C9B0` (hover `#C9B896`) · border `#D8E0D2` · teks utama `#33422B` · teks sekunder `#5C6B52`. ~~**Nol kemunculan** ketujuh hex ini di `src/` hari ini.~~ **[20 Sep 2026 malam, Batch DS 1]** Ketujuh hex kini ada di **`src/kit/tokens.js:29-37`** (+ cermin `kit.css:19-22`) — **dan HANYA di sana**: nol kemunculan di luar `src/kit/` — tetap benar di branch Batch DS 2 (22 halaman admin memakai NAMA token, nol literal sage di halaman; diverifikasi doc-keeper), jadi di `main`/produksi belum satu halaman pun menampilkannya (kit = kode mati), di branch `feat/kit-ds2-adminkit` 22 halaman admin/Foundation sudah.
