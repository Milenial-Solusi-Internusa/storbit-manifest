...
| Halaman **Foundation / Admin Settings / Master Data** (`AdminHub` dan turunannya) | **Kit tunggal `src/kit/`** (A.4.2) — ~~**AdminKit** `src/pages/foundation/admin-settings/` (§6.2)~~ | ✅ **[Batch DS 2 selesai — review ditutup Den 20 Sep 2026 malam]** ke-22 file area ini memakai **kit tunggal `src/kit/`** (impor `…/kit` + `…/kit/tokens`; `kit.css` lewat `main.jsx:4`); AdminKit lama = **kode mati** (nol pemakai; file tetap ada s/d Batch DS 7 — jangan diimpor lagi). Halaman admin/Foundation BARU ikut `src/kit/` — satu-satunya area yang gilirannya sudah lewat (#61 berhenti per halaman). Kalimat "kit resmi" di TD-14 (`08_TECH_DEBT.md`) kini menunjuk `src/kit/`. |
