...
> ⛔ **STATUS KESELURUHAN (diperbarui 20 Sep 2026 malam, sesi kedelapan — revisi kebijakan A.1: `CARD` `#EEF3EA` → `#FFFFFF`, keputusan Den saat review Batch DS 2; status tiap Batch DS TIDAK berubah): KEBIJAKAN SUDAH DIPUTUSKAN; Batch DS 1 SELESAI (kit tunggal ADA di `src/kit/` 12 file + `src/lib/entities.js`); Batch DS 2 (AdminKit → kit, 22 file + `main.jsx`) DIKERJAKAN di branch `feat/kit-ds2-adminkit` dan ⏳ MENUNGGU REVIEW Den — di branch itu 22 halaman admin/Foundation sudah memakai kit sage + `kit.css` diimpor `main.jsx`; tampilan PRODUKSI BELUM BERUBAH (nol halaman produksi ber-sage).** Bagian **A** di bawah memuat keputusan Den 19 Sep 2026: palet aplikasi **sage** + **tiga font resmi** + arah **satu kit tunggal**, plus **rencana Batch DS 1–7 (A.4) yang disetujui 20 Sep 2026 — Batch DS 1 selesai (kode mati) dan review-nya DITUTUP Den (sesi keenam, 20 Sep 2026: #62 dijawab — empat fg kelas STATUS di `tokens.js` digeser minimal ke AA; #61 final), Batch DS 2 ⏳ menunggu review Den (sesi ketujuh — A.4.3), Batch DS 3–7 belum mulai**. **Yang sudah ada di kode `main`/produksi = kit-nya saja** (`tokens.js` sage + komponen + etalase, nol pemakai; `kit.css` belum diimpor `main.jsx`); **di branch Batch DS 2** kit sudah dipakai 22 halaman admin/Foundation (kit lama `admin-settings/kit.jsx`+`tokens.js` tetap ada, nol pemakai di sana — dihapus Batch DS 7); penggantian warna/font di halaman LAIN, pelepasan kit lama, dan dua temuan TD-269/TD-270 **BELUM dikerjakan** (tutup resmi Batch DS 6; `SaveButton` 6 pemanggil TD-270 sudah pindah ke kit di branch Batch DS 2, verifikasi resmi tetap Batch DS 6). Bagian §0–§17 memotret kode `main` @ `660168f` apa adanya (palet navy/oranye lama, tiga keluarga token, font legacy) dan **tetap benar sebagai gambaran halaman yang dilihat user hari ini** — satu-satunya tambahan sejak itu di `main` = `src/kit/` + `src/lib/entities.js` (baris baru di §0 dan §1); anotasi "[Batch DS 2, branch]" di §0/§1/§3.1/§6.2/§17/§18 menandai apa yang sudah berbeda di branch `feat/kit-ds2-adminkit`. Siapa pun (termasuk sesi Claude Code lain) yang membaca dokumen ini: **jangan mengira halaman produksi sudah memakai sage, dan jangan mengimpor `src/kit` ke halaman mana pun di luar giliran Batch DS-nya (A.4.3 — 22 halaman Batch DS 2 sudah lewat gilirannya di branch; sisanya belum)** — #61 **FINAL** (dikonfirmasi Den sesi keenam): berhenti per halaman saat Batch DS-nya selesai, sepenuhnya setelah Batch DS 7.
>
...
## A. Kebijakan resmi (keputusan Den 19 Sep 2026) & arah penyatuan kit — Batch DS 1 ✅ selesai 20 Sep 2026; Batch DS 2 ⏳ menunggu review Den (branch `feat/kit-ds2-adminkit`, 22 halaman admin memakai kit); Batch DS 3–7 ⛔ belum mulai
...
**Status (20 Sep 2026 malam): Batch DS 1 SELESAI — `tokens.js` sage + komponen tunggal + etalase ADA di `src/kit/` (12 file) dan `src/lib/entities.js`; di `main`/produksi masih KODE MATI (nol pemakai; `kit.css` belum diimpor `main.jsx`; etalase tidak di-route). Batch DS 2 (sesi ketujuh) dikerjakan di branch `feat/kit-ds2-adminkit` — ⏳ menunggu review Den: 22 halaman AdminKit + `main.jsx` sudah memakai kit di branch itu (A.4.3). BUKAN klaim penyatuan selesai:** di produksi belum satu halaman pun dimigrasi, di branch baru 22 dari ±110 file; `#144682`/`#1B4D8A`/`#E85A1E`/`#E8703D` masih hidup di kode halaman (sebaran di §3.1 — `#144682` yang ada di `src/kit/tokens.js` = kelas STATUS `TONE.navy`/`DEAL_STATUS.WON`, keputusan #3, bukan identitas; `#E85A1E` **sudah nol di kit** sejak #62 — `TONE.orange` fg kini `#BE4A19`); kit lama tetap dipakai 22 halaman AdminKit (di `main`; di branch Batch DS 2 AdminKit **nol pemakai**, filenya tetap ada s/d Batch DS 7) + 3 halaman v3. **Rencana Batch DS 1–7 (A.4):** Batch DS 1 ✅ **review DITUTUP Den 20 Sep 2026 (sesi keenam)** — kode + screenshot diterima, hasil = #62 (4 fg STATUS digeser ke AA, kontras diukur ulang 0 gagal teks/status) + #61 final, nol permintaan lain; Batch DS 2 ⏳ **menunggu review Den** (sesi ketujuh), Batch DS 3–7 **belum dimulai**, satu Batch DS per giliran atas instruksi Den. Dua temuan kode: **TD-269** (dark green `#2F6B3F` di 5 file + emoji) dan **TD-270** (`SaveButton` AdminKit tidak menunggu Promise) — **keduanya TETAP OPEN**: kit baru hanya membangun jalannya (`src/kit/Button.jsx` `SaveButton` ber-`await`; `src/kit/Data.jsx:125` `<Check/>` Lucide, bukan `'✓'`), file yang bermasalah belum disentuh (kecuali 6 pemanggil `SaveButton` TD-270 yang di branch Batch DS 2 sudah memakai `SaveButton` kit + handler `return { error }` — status TD tetap OPEN); tutup resmi di Batch DS 6.
...
### A.4 Rencana Penyatuan Kit — Batch DS 1 s/d 7 (plan disetujui Den 20 Sep 2026) — Batch DS 1 ✅ selesai 20 Sep 2026 (kit lahir sebagai kode mati); Batch DS 2 ⏳ menunggu review Den (sesi ketujuh); Batch DS 3–7 ⛔ belum mulai

> Istilah **"Batch DS"** (DS = design system) dipakai untuk seluruh rencana ini — **bukan "Wave"** (istilah itu milik Grand Design roadmap terpisah) dan **bukan "Batch" polos** (yang sudah dipakai Batch CRM v3 B1–B6/3C dan Batch 1–3 migrasi TD-14 Agustus 2026). Status per 20 Sep 2026 (malam, sesi ketujuh): **Batch DS 1 selesai — `src/kit/` (12 file) + `src/lib/entities.js` lahir sebagai KODE MATI; Batch DS 2 dikerjakan di branch `feat/kit-ds2-adminkit` (22 file + `main.jsx`, build 2.646 / lint 159) dan ⏳ menunggu review Den — tampilan produksi belum berubah; Batch DS 3–7 belum dimulai** — kolom "Status" tabel A.4.3 adalah satu-satunya laporan kemajuan di bagian ini, sisanya rencana yang sudah disetujui. Kolom "Status" di tabel Batch DS diperbarui saat tiap Batch DS selesai di-review. Baseline sebelum Batch DS 1: `npm run build` clean **2.636 modul** · `npm run lint` **160 (139 error / 21 warning)**.

**A.4.1 Cakupan nyata (diukur 20 Sep 2026, bukan angka kebijakan)**

| Kelompok | Angka kebijakan | Terukur |
|---|---|---|
| Konsumen AdminKit | 22 file | **22** (11 `admin-settings/` + `AdminHub.jsx` + 10 `modules/admin/pages/`), semuanya masih di-route lewat `AdminHub` *[Batch DS 2, branch `feat/kit-ds2-adminkit` — menunggu review: ke-22 file ini + `main.jsx` sudah pindah ke `src/kit/`; `git diff --name-only` = persis 23 file, diverifikasi doc-keeper; AdminKit nol pemakai di branch]* |
...
| `src/lib/entities.js` | `ENTITIES` tunggal (code + UUID + nama legal) — menggantikan dua versi beda bentuk di dua kit *[Batch DS 2, branch: pemakai pertama di luar kit = 5 halaman admin-settings (`ENTITY_IDS = Object.fromEntries(ENTITIES.map(e => [e.code, e.id]))`); versi lokal `PositionsPage.jsx` **tetap** — lihat §1]* |
...
| `SaveButton` berubah perilaku (menunggu server) — pemanggil yang `onSave`-nya tidak mengembalikan Promise akan tetap "sukses instan" | tiap pemanggil dicek di Batch DS 2 *[dikerjakan di branch Batch DS 2: 6 pemanggil TD-270 + 4 alur simpan lain → `SaveButton` kit, handler mengembalikan `{ error }`/`false`; smoke staging: "Tersimpan!" 15–30 ms SETELAH respons PostgREST, offline → "Gagal menyimpan"; alur simpan 5 halaman master + Integrations sengaja tetap `PrimaryBtn` (sudah `await`/tanpa DB); HRGA "Simpan Semua" belum teruji — verifikasi resmi Batch DS 6]* |
...
| Halaman **Foundation / Admin Settings / Master Data** (`AdminHub` dan turunannya) | **AdminKit** — `src/pages/foundation/admin-settings/` (§6.2) | Ditetapkan "kit resmi" untuk area ini di TD-14 (`08_TECH_DEBT.md`); 22 file sudah memakainya **di `main`/produksi**. ⚠️ **[Batch DS 2, branch `feat/kit-ds2-adminkit` — menunggu review Den]** di branch itu ke-22 file sudah memakai **kit tunggal `src/kit/`** dan AdminKit nol pemakai; halaman admin/Foundation BARU yang lahir di branch itu ikut `src/kit/`, yang lahir di `main` masih ikut AdminKit sampai branch masuk. |
...
| **Kit tunggal `src/kit/` + `src/lib/entities.js`** (lahir Batch DS 1, 20 Sep 2026 malam) | ⛔ **JANGAN dipakai halaman apa pun di luar gilirannya — di `main`/produksi masih KODE MATI, nol pemakai; di branch Batch DS 2 (`feat/kit-ds2-adminkit`, menunggu review) pemakainya = 22 halaman admin/Foundation + `main.jsx`.** Halaman masuk kit hanya lewat giliran Batch DS-nya (A.4.3: Batch DS 2 = 22 file AdminKit + `main.jsx` impor `kit.css`; Batch DS 3 = CRM v3; dst.), masing-masing atas instruksi Den. | #61 tetap berlaku — **FINAL, dikonfirmasi Den 20 Sep 2026 (sesi keenam)**: "kit ada" ≠ "instruksi implementasi turun" untuk semua halaman; berhenti per halaman saat Batch DS-nya selesai. Mengimpor `src/kit` sepihak = migrasi di luar rencana, dua kit di satu halaman, dan screenshot "sebelum/sesudah" per Batch DS jadi tak bermakna. Isi kit: A.4.2 + §1 (baris `src/kit/`). |
...
|---|---|---|---|
| `src/pages/foundation/admin-settings/tokens.js` (37 baris) | 14 Jun 2026 | 14 konstanta warna, 3 font, `ENTITIES`, `fmtRp`/`fmtNum` | 22 file: 11 halaman `admin-settings/` + `AdminHub.jsx` + 10 halaman master-data `src/modules/admin/pages/` **[di `main`]** · *[Batch DS 2, branch: **nol pemakai** di luar `kit.jsx` sendiri — ke-22 file pindah ke `src/kit/tokens`; file ini tetap ada s/d Batch DS 7]* |
| `src/pages/foundation/admin-settings/kit.jsx` (573 baris) | 14 Jun 2026 | **24 export** (komentar `crm/v3/kit.jsx:4` yang menyebut "20 export" sudah basi): `Icon`, `KitStyles`, `PageHeader`, `SectionLabel`, `EntitySwitcher`, `Tabs`, `FloatingInput`, `FloatingSelect`, `Toggle`, `NumberStepper`, `Segmented`, `PrimaryBtn`, `OutlineBtn`, `SaveButton`, `Tooltip`, `SlideOver`, `Modal`, `DropZone`, `UploadBox`, `useToast`, `Skel`, `Card`, `KitSelect`, `PillToggle` | sama seperti di atas *[Batch DS 2, branch: nol pemakai — `<KitStyles/>` dicabut 17×, diganti `kit.css` di `main.jsx`; `SaveButton` lama tak lagi dipanggil siapa pun di branch]* |
...
| **`src/kit/`** (12 file, ~~2.335~~ ~~2.345~~ **2.347** baris — `wc -l` sesudah revisi `CARD` 20 Sep malam, sesi kedelapan): `tokens.js` (~~166~~ ~~175~~ **177**) · `kit.css` (56) · `Button.jsx` (245) · `Form.jsx` (372) · `Layout.jsx` (268) · `Data.jsx` (431) · `Overlay.jsx` (154) · `Upload.jsx` (108) · `Icon.jsx` (64) · `useToast.jsx` (56) · `index.js` (~~31~~ **32**) · `__demo/KitGallery.jsx` (384) | 20 Sep 2026 (Batch DS 1) | **Kit tunggal ber-palet sage + tiga font** (A.4.2): 9 hex A.1 + turunan berumus; palet STATUS/DATA kelas terpisah (`SEMANTIC`/`TONE`/`DEAL_STATUS`; sejak #62 [20 Sep, sesi keenam] empat fg `ok`/`warn`/`slate`/`orange` digeser minimal ke AA 4,5:1 — seluruh pasangan teks/status kit lolos AA); **31 export** (komponen + `Icon` + `useToast`) + **5 alias** nama lama (`PrimaryBtn`/`OutlineBtn`/`KitSelect`/`Skel`/`Tabs`, dicabut Batch DS 7); `FormSheet`/`Notebook`/`StatusBar`/`ListView` = **salinan v3** berwarna sage (file v3 asli tak disentuh, dihapus Batch DS 3); `Icon` registri 66 nama identik AdminKit; `kit.css` prefiks `nk-` (≠ `ak-`); `SaveButton` menunggu Promise (arah TD-270) | **Di `main`/produksi: NOL — KODE MATI.** Tidak diimpor satu file pun di luar `src/kit/` (satu-satunya `from '../kit'` lain = `crm/v3/__demo/CrmV3DemoPage.jsx:24` → `crm/v3/kit.jsx`); `kit.css` belum diimpor `main.jsx`; etalase tidak di-route. **Di branch Batch DS 2 (`feat/kit-ds2-adminkit`, 20 Sep malam, menunggu review Den): 22 file** (11 `admin-settings/*` + `AdminHub.jsx` + 10 `modules/admin/pages/*`) mengimpor `…/kit` + `…/kit/tokens`, `main.jsx` mengimpor `kit.css`; build 2.646 modul (+10 = kit masuk graph; etalase tetap di luar). Tampilan produksi belum berubah. |
| `src/lib/entities.js` (39 baris) | 20 Sep 2026 (Batch DS 1) | `ENTITIES` tunggal `{ code, id: UUID companies.id, name legal }` (SOA "PT Stuja Orbit Abadi") + `ENTITY_CODES` + `ENTITY_BY_CODE`/`ENTITY_BY_ID` (frozen) + `entityByCode`/`entityById`/`entityCodeOf` | `src/kit/Form.jsx` (`EntitySwitcher`, banding `e.code`) saja **[di `main`]** · *[Batch DS 2, branch: +5 halaman admin-settings (ApprovalWorkflows · DocumentSettings · EntitySettings · FinanceDefaults · Notifications) lewat `ENTITY_IDS = Object.fromEntries(ENTITIES.map(e => [e.code, e.id]))`]*. Dua versi lama tetap hidup s/d Batch DS 7 (`admin-settings/tokens.js:29` id = kode tanpa UUID · `crm/v3/tokens.js:147` id = UUID); versi lokal lain yang juga harus dilebur: `PositionsPage.jsx:46` (UUID, nama tanpa "PT" — ~~Batch DS 2~~ ⚠️ **TIDAK disentuh Batch DS 2** [branch: `:40-45`, `ENTITY_IDS = ENTITIES.map(e => e.id)` untuk `.in('company_id', …)`, beda bentuk; `handleSave` byte-identik per TD-14] — giliran barunya perlu konfirmasi Den: Batch DS 7 bersama dua versi lama, atau lebih awal) · **`CRMReportPage.jsx:61`** `["MSI","JCI","SOA"]` (array kode — *temuan doc-keeper 20 Sep malam, belum disebut rencana; kandidat Batch DS 4c*) |
...
**Resmi sejak 19 Sep 2026 (Bagian A.1; CARD direvisi 20 Sep malam):** latar halaman `#F6F8F3` · latar kartu `#FFFFFF` (semula `#EEF3EA`) · aksen utama `#C3D9B8` (hover `#AFC9A0`) · aksen kedua `#D7C9B0` (hover `#C9B896`) · border `#D8E0D2` · teks utama `#33422B` · teks sekunder `#5C6B52`. ~~**Nol kemunculan** ketujuh hex ini di `src/` hari ini.~~ **[20 Sep 2026 malam, Batch DS 1]** Ketujuh hex kini ada di **`src/kit/tokens.js:31-39`** *(`:29-37` sebelum revisi `CARD` menambah 2 baris komentar header)* (+ cermin `kit.css:19-22`) — **dan HANYA di sana**: nol kemunculan di luar `src/kit/` *[⚠️ sejak `CARD` = `#FFFFFF` (sesi kedelapan) klaim ini berlaku untuk **ENAM hex sage khas**; `#FFFFFF` sebagai putih biasa ada di **35 file** lain di `src/` dan **bukan penanda grep** — diukur doc-keeper]* — tetap benar di branch Batch DS 2 (22 halaman admin memakai NAMA token, nol literal sage di halaman; diverifikasi doc-keeper), jadi di `main`/produksi belum satu halaman pun menampilkannya (kit = kode mati), di branch `feat/kit-ds2-adminkit` 22 halaman admin/Foundation sudah.
...
### 6.2 AdminKit — `src/pages/foundation/admin-settings/` *(keadaan `main`; di branch Batch DS 2 `feat/kit-ds2-adminkit` — menunggu review Den — kit ini **nol pemakai**: ke-22 halamannya memakai `src/kit/`, `<KitStyles/>` tak dirender lagi; file kit tetap ada s/d Batch DS 7)*

Import: `import { PageHeader, Card, FloatingInput, … } from '../../../pages/foundation/admin-settings/kit'` dan token dari `…/tokens`. **`<KitStyles />` wajib dirender sekali di halaman** (keyframes, `.ak-skel`, `.ak-spin`, `.ak-scroll`, `.ak-rise`). *[Batch DS 2, branch: halaman yang sudah pindah ke `src/kit/` tidak merender `<KitStyles/>` (dicabut 17×) — gantinya `kit.css` diimpor sekali di `main.jsx`, kelas `nk-*`.]*
...
| `SaveButton` | tombol simpan 3-keadaan idle→saving→saved | `onSave` `label="Simpan"` `variant="primary"/"navy"`; ⚠️ **tidak menunggu Promise** — `onSave` dipanggil setelah jeda 950ms, "Tersimpan!" 2s lalu reset, apa pun hasil server (**TD-270**; *di branch Batch DS 2 keenam pemanggilnya sudah memakai `SaveButton` kit `src/kit/Button.jsx` yang menunggu Promise — komponen lama ini tak berubah dan nol pemakai di sana*) | `<SaveButton onSave={handleSave}/>` |
...
// AdminKit (halaman Foundation / master data) — keadaan `main`; di branch Batch DS 2 (`feat/kit-ds2-adminkit`) ke-22 halaman ini SUDAH memakai bentuk "Kit tunggal" di bawah
...
// Kit tunggal (src/kit/) — di `main` KODE MATI sejak Batch DS 1 (20 Sep 2026); di branch Batch DS 2 dipakai 22 halaman admin/Foundation: JANGAN diimpor halaman mana pun
// di luar giliran Batch DS-nya (A.4.3, #61 — FINAL, dikonfirmasi Den 20 Sep 2026). Bentuk impor yang dipakai Batch DS 2 (22 halaman, branch) dan Batch DS 3+ (path relatif —
...
// `kit.css` diimpor SEKALI di src/main.jsx (Batch DS 2 — sudah di branch `feat/kit-ds2-adminkit`, `main.jsx:4`) — bukan per halaman, bukan <KitStyles/>.
// Bersama
