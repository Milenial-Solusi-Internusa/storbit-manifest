...
> ⛔ **STATUS KESELURUHAN (diperbarui 20 Sep 2026 malam, sesi kesembilan — review Batch DS 2 DITUTUP Den): KEBIJAKAN SUDAH DIPUTUSKAN; Batch DS 1 SELESAI (kit tunggal ADA di `src/kit/` 12 file + `src/lib/entities.js`); Batch DS 2 (AdminKit → kit, 22 file + `main.jsx`) ✅ SELESAI — review DITUTUP Den 20 Sep 2026 malam (sesi kesembilan; hasil review = revisi `CARD` → `#FFFFFF` [sesi kedelapan], nol permintaan perubahan lain): di `main` 22 halaman admin/Foundation memakai kit sage + `kit.css` diimpor `main.jsx:4`; tampilan PRODUKSI untuk 22 halaman itu berubah begitu deploy `main` berikutnya — halaman LAIN belum ber-sage; ⚠️ satu catatan terbuka BELUM LOLOS: alur simpan HRGA "Simpan Semua" belum pernah dites (A.4.3 baris Batch DS 2); Batch DS 3–7 ⛔ belum mulai.** Bagian **A** di bawah memuat keputusan Den 19 Sep 2026: palet aplikasi **sage** + **tiga font resmi** + arah **satu kit tunggal**, plus **rencana Batch DS 1–7 (A.4) yang disetujui 20 Sep 2026 — Batch DS 1 selesai (kode mati) dan review-nya DITUTUP Den (sesi keenam, 20 Sep 2026: #62 dijawab — empat fg kelas STATUS di `tokens.js` digeser minimal ke AA; #61 final), Batch DS 2 ✅ selesai (review ditutup Den sesi kesembilan — A.4.3), Batch DS 3–7 belum mulai**. **Yang sudah ada di kode `main` = kit + 22 halaman admin/Foundation yang memakainya** (Batch DS 2; `kit.css` diimpor `main.jsx:4`; kit lama `admin-settings/kit.jsx`+`tokens.js` tetap ada, nol pemakai — dihapus Batch DS 7); penggantian warna/font di halaman LAIN, pelepasan kit lama, dan dua temuan TD-269/TD-270 **BELUM dikerjakan** (tutup resmi Batch DS 6; `SaveButton` 6 pemanggil TD-270 sudah pindah ke kit sejak Batch DS 2, verifikasi resmi tetap Batch DS 6 — termasuk HRGA "Simpan Semua" yang belum teruji). Bagian §0–§17 memotret kode `main` @ `660168f` apa adanya (palet navy/oranye lama, tiga keluarga token, font legacy) dan **tetap benar sebagai gambaran halaman di luar 22 halaman Batch DS 2** — yang berubah di `main` sejak itu = `src/kit/` + `src/lib/entities.js` (baris baru di §0 dan §1) dan 22 halaman admin/Foundation + `main.jsx` (Batch DS 2); anotasi "[Batch DS 2 …]" di §0/§1/§3.1/§6.2/§17/§18 (ditulis saat pekerjaan masih di branch `feat/kit-ds2-adminkit`) kini menggambarkan keadaan `main`. Siapa pun (termasuk sesi Claude Code lain) yang membaca dokumen ini: **jangan mengira semua halaman sudah memakai sage (baru 22 halaman admin/Foundation), dan jangan mengimpor `src/kit` ke halaman mana pun di luar giliran Batch DS-nya (A.4.3 — 22 halaman Batch DS 2 sudah lewat gilirannya; sisanya belum)** — #61 **FINAL** (dikonfirmasi Den sesi keenam): berhenti per halaman saat Batch DS-nya selesai, sepenuhnya setelah Batch DS 7.
>
...
## A. Kebijakan resmi (keputusan Den 19 Sep 2026) & arah penyatuan kit — Batch DS 1 ✅ selesai 20 Sep 2026; Batch DS 2 ✅ selesai — review ditutup Den 20 Sep 2026 malam (22 halaman admin memakai kit); Batch DS 3–7 ⛔ belum mulai
...
| Latar kartu | `#FFFFFF` | kartu, panel, surface — **direvisi Den 20 Sep 2026 malam (semula `#EEF3EA`)**: kartu **putih polos**; pemisah dari latar halaman cukup lewat **border `LINE`**; sage tetap tampil lewat teks, ikon, dan aksen, bukan lewat latar kartu |
...
- **Catatan revisi CARD (20 Sep 2026 malam, saat review Batch DS 2):** delapan token lain **tidak berubah**. Token turunan yang dihitung dari `CARD` ikut dihitung ulang di `src/kit/tokens.js` dengan rumus yang sama: `LINE_SOFT` `#E3EADE`→`#ECF0E9`, `HEAD_BG` `#E5EBE0`→`#EFF3ED` (thead/header kartu, `SEMANTIC.neutral.bg`), `DISABLED_BG` `#E1E8DC`→`#E8ECE4`; `INPUT_BG` tetap `= BG` (`#F6F8F3`) sehingga input kini bidang isian sage samar di atas kartu putih (1,07:1, tetap dibedakan border + fokus), bukan lagi "timbul lebih terang"; `ROW_HOVER` tetap (rumusnya dari `BG`). Kontras diukur ulang: seluruh teks/status lolos AA — INK di CARD 10,74 · INK_SOFT 5,71 · INK_FAINT 3,43 (placeholder) · INK_SOFT di HEAD_BG 5,09; kartu vs halaman 1,07:1, border vs kartu 1,35:1.
...
**Status (20 Sep 2026 malam, sesi kesembilan): Batch DS 1 SELESAI — `tokens.js` sage + komponen tunggal + etalase ADA di `src/kit/` (12 file) dan `src/lib/entities.js` (etalase tidak di-route). Batch DS 2 SELESAI — review DITUTUP Den 20 Sep 2026 malam: 22 halaman AdminKit + `main.jsx` memakai kit di `main` (A.4.3; dikerjakan di branch `feat/kit-ds2-adminkit`). BUKAN klaim penyatuan selesai:** baru 22 dari ±110 file yang dimigrasi; `#144682`/`#1B4D8A`/`#E85A1E`/`#E8703D` masih hidup di kode halaman (sebaran di §3.1 — `#144682` yang ada di `src/kit/tokens.js` = kelas STATUS `TONE.navy`/`DEAL_STATUS.WON`, keputusan #3, bukan identitas; `#E85A1E` **sudah nol di kit** sejak #62 — `TONE.orange` fg kini `#BE4A19`); kit lama: AdminKit **nol pemakai** sejak Batch DS 2 (filenya tetap ada s/d Batch DS 7), kit v3 masih dipakai 3 halaman. **Rencana Batch DS 1–7 (A.4):** Batch DS 1 ✅ **review DITUTUP Den 20 Sep 2026 (sesi keenam)** — kode + screenshot diterima, hasil = #62 (4 fg STATUS digeser ke AA, kontras diukur ulang 0 gagal teks/status) + #61 final, nol permintaan lain; Batch DS 2 ✅ **review DITUTUP Den 20 Sep 2026 malam (sesi kesembilan)** — hasil = revisi `CARD` → `#FFFFFF` (sesi kedelapan), nol permintaan lain; ⚠️ catatan terbuka: HRGA "Simpan Semua" belum teruji (A.4.3); Batch DS 3–7 **belum dimulai**, satu Batch DS per giliran atas instruksi Den. Dua temuan kode: **TD-269** (dark green `#2F6B3F` di 5 file + emoji) dan **TD-270** (`SaveButton` AdminKit tidak menunggu Promise) — **keduanya TETAP OPEN**: kit baru hanya membangun jalannya (`src/kit/Button.jsx` `SaveButton` ber-`await`; `src/kit/Data.jsx:125` `<Check/>` Lucide, bukan `'✓'`), file yang bermasalah belum disentuh (kecuali 6 pemanggil `SaveButton` TD-270 yang sejak Batch DS 2 sudah memakai `SaveButton` kit + handler `return { error }` — status TD tetap OPEN); tutup resmi di Batch DS 6.

**Aturan selama masa tunggu — DISETUJUI Den 20 Sep 2026 (#61, bukan lagi asumsi kerja):** halaman/fitur baru yang dibuat sementara **mengikuti kit/token yang sudah ada sekarang** (tabel §0), **BUKAN langsung memakai sage duluan** — supaya tidak lahir palet KELIMA di kode (empat yang hidup: v3 `#144682`, AdminKit/shell `#1B4D8A`, warm-beige lokal, ungu/serif Storbit web). Halaman yang lahir di masa tunggu ikut dimigrasi bersama yang lain saat penyatuan. **✅ TAFSIRAN FINAL (dikonfirmasi Den 20 Sep 2026, sesi keenam — jangan ditafsirkan ulang di sesi lain): keberadaan `src/kit/` sejak Batch DS 1 TIDAK mengubah aturan ini.** Kit tunggal hanya boleh diimpor sebuah halaman **pada giliran Batch DS halaman itu** (tabel A.4.3); halaman/fitur BARU yang lahir sebelum gilirannya tetap memakai kit/token tetangganya (tabel §0), lalu ikut dimigrasi pada Batch DS modulnya. Mengimpor `src/kit` ke halaman di luar giliran = migrasi di luar rencana (dua kit di satu halaman) — ditolak saat review. Aturan ini berhenti dengan sendirinya per halaman begitu Batch DS-nya selesai, dan sepenuhnya setelah Batch DS 7.
...
### A.4 Rencana Penyatuan Kit — Batch DS 1 s/d 7 (plan disetujui Den 20 Sep 2026) — Batch DS 1 ✅ selesai 20 Sep 2026 (kit lahir sebagai kode mati); Batch DS 2 ✅ selesai — review ditutup Den 20 Sep 2026 malam (sesi kesembilan); Batch DS 3–7 ⛔ belum mulai

> Istilah **"Batch DS"** (DS = design system) dipakai untuk seluruh rencana ini — **bukan "Wave"** (istilah itu milik Grand Design roadmap terpisah) dan **bukan "Batch" polos** (yang sudah dipakai Batch CRM v3 B1–B6/3C dan Batch 1–3 migrasi TD-14 Agustus 2026). Status per 20 Sep 2026 (malam, sesi kesembilan): **Batch DS 1 selesai — `src/kit/` (12 file) + `src/lib/entities.js`; Batch DS 2 selesai — review ditutup Den 20 Sep 2026 malam (22 file + `main.jsx` di `main`, build 2.646 / lint 159 = 138/21; dikerjakan di branch `feat/kit-ds2-adminkit`); Batch DS 3–7 belum dimulai** — kolom "Status" tabel A.4.3 adalah satu-satunya laporan kemajuan di bagian ini, sisanya rencana yang sudah disetujui. Kolom "Status" di tabel Batch DS diperbarui saat tiap Batch DS selesai di-review. Baseline sebelum Batch DS 1: `npm run build` clean **2.636 modul** · `npm run lint` **160 (139 error / 21 warning)**.

**A.4.1 Cakupan nyata (diukur 20 Sep 2026, bukan angka kebijakan)**

| Kelompok | Angka kebijakan | Terukur |
|---|---|---|
| Konsumen AdminKit | 22 file | **22** (11 `admin-settings/` + `AdminHub.jsx` + 10 `modules/admin/pages/`), semuanya masih di-route lewat `AdminHub` *[Batch DS 2 ✅ selesai (review ditutup Den 20 Sep 2026 malam): ke-22 file ini + `main.jsx` sudah pindah ke `src/kit/` (persis 23 file, diverifikasi doc-keeper); AdminKit nol pemakai di `main` — diverifikasi ulang doc-keeper sesi kesembilan: 22 file mengimpor `…/kit`, nol impor `admin-settings/kit` maupun `admin-settings/tokens` di luar komentar]* *[21 Sep 2026 — Batch FS Fase 1: `AdminSettingsHub.jsx` (hub lama yatim, termasuk 11 `admin-settings/` di atas) DIHAPUS → pengimpor `src/kit` di area ini kini **21** (10 `admin-settings/*Page` + `AdminHub.jsx` + 10 `modules/admin/pages/`); angka 22 tetap benar sebagai cakupan Batch DS 2 saat itu]* |
| Konsumen kit CRM v3 | 3 halaman | **3 + `DealPanels.jsx`** (31 hex, dipakai Detail Deal) |
| Token lokal `const C/D/S/T` | 52 file | **52**, plus 11 file `const PASTEL` = **63 file / ±41.600 baris** (CRM 21 file 16,5k · logistics 8 · components 12 · assets 4 · `App.jsx` 5.613 baris) |
| File lain yang hardcode hex tanpa objek token | — | **+50 file** (Products/ProductDetail 79/63 hex, HRGA 3, BNF 2, MOM 4, procurement 4, Home, PenerimaanBarang, AddAsset, …) |
| Tailwind palet default (`bg-blue-500` dsb.) | — | **194 pemakaian / 6 file** (BNF, Meeting Mingguan, Briefing Harian, BNFOrgRoles, StatusCatalog, BnfActionItemsChecklist) — keluarga warna ke-5 |

Penyatuan **penuh** menyentuh ±110 file. Dua fakta palet yang menentukan desain kit (WCAG, dihitung): teks putih di atas aksen sage **gagal** (1,5–1,95:1) → semua pola "solid navy/oranye + teks putih" menjadi **latar aksen + teks `#33422B`** (7,1:1); kartu vs halaman ~~**1,05:1**~~ **1,07:1** dan border vs kartu ~~1,2:1~~ **1,35:1** *[diukur ulang 20 Sep malam setelah `CARD` direvisi putih (A.1); dua angka lama = keadaan `CARD #EEF3EA`]* → pemisah kartu dari halaman = border `LINE` (+ bayangan turunan), dan fokus ring memakai `#5C6B52`/`#33422B` (aksen hanya 1,4:1 terhadap halaman).

**A.4.2 Struktur kit tunggal — `src/kit/`**

| File | Isi |
|---|---|
| `tokens.js` | SATU file token (konstanta polos, nol JSX) — lihat rincian di bawah |
| `kit.css` | keyframes, scrollbar, focus-ring; diimpor **sekali** di `main.jsx` (menggantikan `<KitStyles/>` per halaman) |
| `Button.jsx` | `Button` (variant `primary`/`secondary`/`outline`/`ghost`/`danger` · size `md` 40 / `sm` 32 / `xs` 28 · `icon` · `loading` · `disabled`), `SaveButton` (menunggu Promise → menutup TD-270), `IconButton` |
| `Form.jsx` | `FloatingInput`, `FloatingSelect`, `Select` (ringkas 44px), `Textarea`, `SearchInput`, `Toggle`, `NumberStepper`, `Segmented`, `PillToggle`, `EntitySwitcher` |
| `Layout.jsx` | `PageHeader`, `SectionLabel`, `Card`, `EmptyState`, `Skeleton`, **`DocNo`** *[diselaraskan dengan kode 20 Sep malam: tinggal di sini (`Layout.jsx:136`), bukan di `Data.jsx`, supaya impor Data→Layout satu arah; `Data.jsx:29` me-re-export]*, `FormSheet` (pindahan v3), `Notebook` (pindahan v3; menyerap `Tabs` AdminKit: +`icon`/+`dot`) |
| `Data.jsx` | `ListView` (pindahan v3, mode table/lanes), `Badge`, ~~`DocNo`~~ *(re-export dari `Layout.jsx`, lihat baris di atas)*, `StatusBar` (pindahan v3) |
| `Overlay.jsx` | `Modal`, `SlideOver`, `Tooltip` |
| `Upload.jsx` | `DropZone`, `UploadBox` |
| `Icon.jsx` | registry nama→Lucide (kompatibilitas 13 halaman admin; komponen baru tetap impor Lucide langsung) |
| `useToast.jsx` | hook toast lokal (10 halaman admin masih memakainya); file hanya meng-export hook |
| `index.js` | barrel komponen saja |
| `__demo/KitGallery.jsx` | etalase semua komponen, **tidak di-route** (pengganti Storybook; sumber screenshot) |
| `src/lib/entities.js` | `ENTITIES` tunggal (code + UUID + nama legal) — menggantikan dua versi beda bentuk di dua kit *[Batch DS 2 (selesai): pemakai pertama di luar kit = 5 halaman admin-settings (`ENTITY_IDS = Object.fromEntries(ENTITIES.map(e => [e.code, e.id]))`); versi lokal `PositionsPage.jsx` **tetap** — lihat §1]* |
| `src/modules/crm/dealStatus.js` | `STATUS_LABEL`, `DEAL_STATUS_ORDER`, `LIFECYCLE_ORDER` — kosakata CRM keluar dari kit (kit harus buta sumbu). ⛔ **Belum ada** (lahir Batch DS 3; per 20 Sep malam `ls` kosong — ketiganya masih di `crm/v3/tokens.js`) |

Isi `tokens.js`:

| Kelompok | Isi |
|---|---|
| Palet identitas (9, persis A.1) | `BG #F6F8F3` · `CARD #FFFFFF` (revisi 20 Sep malam, semula `#EEF3EA`) · `ACCENT #C3D9B8` / `ACCENT_HOVER #AFC9A0` · `ACCENT_2 #D7C9B0` / `ACCENT_2_HOVER #C9B896` · `LINE #D8E0D2` · `INK #33422B` · `INK_SOFT #5C6B52` |
| Turunan (dihitung dari 9 di atas, **nol hue baru**, rumus ditulis di komentar) | `INK_FAINT` (placeholder/hint), `LINE_SOFT`, `ROW_HOVER`, `HEAD_BG` (thead/header kartu), `INPUT_BG` (= `BG`; ~~supaya input timbul di atas kartu~~ *sejak `CARD` putih [20 Sep malam] = bidang isian sage samar 1,07:1 di atas kartu, dibedakan border `LINE` + fokus*), `DISABLED_BG`/`DISABLED_INK`, `FOCUS_RING` (rgba `INK_SOFT` .35, 3px), `BACKDROP` (rgba `INK` .45), `SHADOW_1`/`SHADOW_2` (rgba `INK`) |
| Font | `FONT_HEAD` Montserrat · `FONT_BODY` Inter · `FONT_MONO` IBM Plex Mono — nilai persis yang ada sekarang |
| Skala | `SP {4,8,12,16,20,24,32}` · `RADIUS {sm 6, md 10, lg 14, xl 18, pill 999}` (AdminKit 11→10, 16→14) · `Z` (tooltip 60 · slide-over 80 · modal 90 · popover 150 · toast 200 · confirm 9999) · `MOTION` (ease `cubic-bezier(.22,1,.36,1)`, .12/.2/.3s) |
| **Palet STATUS/DATA — kelas terpisah (#60 b)** | `SEMANTIC` ok/warn/danger/info/neutral (trio fg/bg/bd; bg/bd dari warm-beige yang sudah dipakai 40+ file, fg `ok`/`warn` digeser #62, `neutral` diturunkan ulang dari sage) · `TONE` slate/orange/navy/brick (v3; bg/bd apa adanya, fg `slate`/`orange` digeser #62) · `DEAL_STATUS` 7 status (`STATUS_META` yang sudah lolos AA, WON navy) · helper `toneOf`/`closedFrom`/`isClosedStage`. ✅ **#62 DIJAWAB Den 20 Sep 2026 (sesi keenam): empat fg DIGESER minimal demi AA 4,5:1 teks badge — `SEMANTIC.ok` → `#2D794D` (4,53) · `SEMANTIC.warn` → `#8E620D` (4,58) · `TONE.slate` → `#676D7B` (4,54) · `TONE.orange` → `#BE4A19` (4,57)**; bg/bd tetap. Diukur ulang atas `tokens.js` final: seluruh pasangan teks/status kit **lolos AA (0 gagal)**. Konsekuensi: `SEMANTIC.ok/warn` kit ≠ `C.ok/C.warn` warm-beige lama dan `TONE.slate/orange` kit ≠ `crm/v3/tokens.js` (beda 3–18 % lebih gelap, bukan hue baru) — file lama ikut nilai kit saat gilirannya; sejak ini **nol `#E85A1E` di kit** (oranye brand lama hanya hidup di kode lama), `#144682` tetap sebagai warna STATUS. |
| *[+ ada di kode sejak Batch DS 1, belum tercantum di rencana — disusul doc-keeper 20 Sep malam]* | `SHADOW_DROP` (`drop-shadow` segmen aktif StatusBar; clip-path butuh filter) · `WHITE` (**hanya** teks di atas warna STATUS solid, mis. penanda WON) · **`SIZE {btnMd 40, btnSm 32, btnXs 28, input 44, inputFloating 56}`** (keputusan #7) · `STAGE_TONE`/`CLOSED_STAGES` (peta id sumbu → tone, disalin v3; `toneOf` kini juga menerima nama `SEMANTIC` & objek trio) · `fmtRp`/`fmtNum` (kompatibilitas 3 halaman admin yang mengimpornya dari `tokens.js` lama; rumah permanen = keputusan Batch DS 7) |

Peta lama → baru untuk konsumen: `NAVY`/`ORANGE` (bg tombol) → `ACCENT`/`ACCENT_2` **+ teks ikut jadi `INK`** · `CREAM`/`SURFACE`/`SURFACE_2` → `CARD`/`INPUT_BG`/`HEAD_BG` · `INK`/`INK_SOFT`/`MUTED`/`FAINT` → `INK`/`INK_SOFT`/`INK_SOFT`/`INK_FAINT` · `LINE`/`LINE_SOFT`/`ROW_HOVER` → sama nama · `DANGER`/`GREEN` → `SEMANTIC.danger.fg`/`SEMANTIC.ok.fg` · `PrimaryBtn`/`OutlineBtn` (2 kit) + `ghostBtn` → `Button variant=…` · `KitSelect` → `Select` · `Skel` → `Skeleton` · `Tabs` → `Notebook` · `KitStyles` → `kit.css`. Selama migrasi kit meng-export **alias nama lama** (`PrimaryBtn` = `Button variant="primary"`, dst.) supaya diff tiap halaman cuma path impor + nama token; alias dicabut di Batch DS 7.

**A.4.3 Batch DS (tiap Batch DS = branch sendiri, satu commit, review satu-satu; komponen v3 disalin ke kit sejak Batch DS 1 dan file aslinya baru dihapus di Batch DS 3 — dua salinan selama Batch DS 1–2 disengaja)**

| Batch DS | Cakupan & file | Verifikasi | Staging dulu? | Status |
|---|---|---|---|---|
| **1 — Kit tunggal lahir, belum dipakai** | 12 file baru `src/kit/` + `src/lib/entities.js` (+ `__demo/KitGallery.jsx`); nol file lama disentuh; `SaveButton` langsung benar (await Promise, jalur error). **Sekaligus ambil screenshot "SEBELUM" semua halaman kunci (A.4.4) selagi tampilan masih lama.** | build (+modul), lint net-zero (file baru 0 error), skrip kontras dijalankan ulang atas nilai token final, screenshot KitGallery (mount sementara, tanpa DB) | Tidak — kode mati | ✅ **selesai 20 Sep 2026** — 13 file baru (`src/kit/` 12 + `src/lib/entities.js`), nol file lama disentuh; build tetap 2.636 modul (kit belum diimpor siapa pun), lint tetap 160 (139/21), file baru 0 error; kontras diukur ulang atas `tokens.js` final (laporan: `PROGRESS.md` 2026-09-20); screenshot KitGallery + "SEBELUM" 16 halaman tersimpan di luar repo. **Review DITUTUP Den 20 Sep 2026 (sesi keenam):** kode + screenshot diterima; hasil review = **#62 DIJAWAB** (4 fg kelas STATUS `tokens.js` digeser minimal ke AA — `ok #2D794D` · `warn #8E620D` · `slate #676D7B` · `orange #BE4A19`; kontras diukur ulang 0 gagal teks/status; gate tetap 2.636 / 160 / kit 0-0) + **#61 FINAL**; nol permintaan perubahan lain. Berikutnya **Batch DS 2**, hanya atas instruksi Den. |
| **2 — AdminKit → kit** | 22 file (11 `admin-settings/*` — `<KitStyles/>` dicabut, `useToast` tetap; `AdminHub.jsx`; 10 `modules/admin/pages/*`) + `main.jsx` +1 baris impor `kit.css`. `admin-settings/kit.jsx`+`tokens.js` dibiarkan (dihapus Batch DS 7). | build/lint; `git diff --stat` = persis 23 file; screenshot AdminHub · Companies · Branches (modal CRUD) · Entity Settings · Approval Workflows · Finance Defaults; smoke 1 alur simpan per jenis (`SaveButton` kini async — pesan sukses hanya setelah RPC selesai) | Boleh langsung `main` (admin/super_admin, trafik rendah); alur simpan dites di preview dulu | ✅ **selesai — review DITUTUP Den 20 Sep 2026 malam (sesi kesembilan)**, hasil review = revisi `CARD` → `#FFFFFF` (sudah diterapkan). ⚠️ **Catatan terbuka, BUKAN lolos:** alur simpan HRGA **"Simpan Semua"** (`ApprovalWorkflowsPage`, `hrga_approval_configs.upsert`) **belum pernah dites** — staging MSI nol `hrga_request_types` sehingga tombolnya tak dirender; wajib dicoba manual begitu ada datanya di staging atau setelah produksi (tombol/handler sudah pola yang sama dengan 5 alur yang lolos smoke). *Jejak saat review:* dikerjakan di branch `feat/kit-ds2-adminkit`: 22 file + `main.jsx` (persis 23 file); build 2.646 modul (+10 = file kit ikut bundel), lint 159 (138/21) = −1 dari baseline, nol error baru per file; `<KitStyles/>` dicabut 17×, `useToast` tetap; ~~5~~ **6** halaman ber-`SaveButton` (= 6 pemanggil TD-270; koreksi hitung doc-keeper, `git show HEAD` 6 file) + 4 alur simpan lain / 5 tombol (Simpan Workflow, Simpan Rule, Simpan Rekening, Simpan penanda tangan, baris penomoran) kini memakai `SaveButton` kit — 11 `<SaveButton` di 7 file — dengan handler yang mengembalikan `{ error }`; smoke staging: 5 simpan sukses "Tersimpan!" 15–30 ms SETELAH respons PostgREST, offline → "Gagal menyimpan"; screenshot sebelum/sesudah 6 halaman tersimpan di luar repo. Den mencoba sendiri di Preview deployment branch (gate #5). **[sesi kedelapan, 20 Sep malam]** hasil review = **revisi kebijakan A.1 `CARD` → `#FFFFFF`** (keputusan Den), diterapkan di branch yang sama pada **2 file kit saja** — `src/kit/tokens.js` (`CARD` + turunan `LINE_SOFT`/`HEAD_BG`/`DISABLED_BG` dihitung ulang, komentar `INPUT_BG` & header) + `kit.css` (`--nk-line-soft`); nol file halaman berubah, 22 halaman ikut otomatis lewat token. Gate diukur ulang: `npx eslint src/kit` 0/0 · build tetap **2.646** modul; kontras 0 gagal teks/status (INK di CARD 10,74 · INK_SOFT 5,71). Screenshot ulang 6 halaman (AdminHub · Companies · Branches + 2 modal · Entity Settings · Approval Workflows + HRGA · Finance Defaults) × (1280 + 375) dengan kartu putih: nol page/console error (laporan sesi; 18 PNG + `_errors.json` kosong ada di luar repo, dilihat doc-keeper). **[21 Sep 2026 — Batch FS Fase 1: `AdminSettingsHub.jsx` (1 dari 22 file Batch DS 2; hub lama yatim, bukan `AdminHub.jsx`) dihapus → pemakai kit di area ini tinggal 21 halaman + `main.jsx`; 22 tetap angka cakupan Batch DS 2. Nol perubahan lain pada kit/halaman.]** |
| **3 — Kit CRM v3 → kit** | `DealDetailPage`, `DealPanels`, `InquiryListPage` (+ `C` lokal & `STATUS_META`→`DEAL_STATUS`), `PipelineKanbanPage`; `v3/tokens.js` kosakata → `crm/dealStatus.js`; `v3/Chatter.jsx` (0 pemakai) & `__demo` dihapus; `InquiryChatter.jsx` di-restyle; folder `crm/v3/` dihapus | build/lint; screenshot Inquiry List, Detail Deal (3 tab + chevron + aside), Pipeline (lanes + lajur tertutup); smoke: buka deal, ganti tab, filter saved-view, komentar | **YA** — halaman harian sales | ⛔ belum mulai |
| **4a — `src/components/` (12)** | 6 picker, Login, AuthGate, ErrorBoundary, CompanySwitcher, CustomFieldsSection, ProfileMiniView, **ConfirmModal** (23 pemakai; tetap di tempat, restyle) | build/lint; screenshot Login, satu form ber-picker, ConfirmModal 3 varian | Preview dulu (dipakai lintas modul) | ⛔ belum mulai |
| **4b — admin sisa (±13)** | SchemaManager, BulkEditPrice, AdminFormModal, AdminPageHeader, Loading/Empty/ErrorState, `userAccessTokens.js` (PASTEL), UserAccess/UserEdit/userAccessShared/RoleDefaults, **Products/ProductDetail** (79/63 hex; 2 titik TD-269) | build/lint; screenshot User Access, User Edit, Products | `main` | ⛔ belum mulai |
| **4c — CRM (21 file, 3 giliran)** | 4c-i 12 halaman kecil (List/Form/Modal) · 4c-ii CustomerDetail 1.940, QuotationForm 1.650, Activities, QuotationDetail, CRMReport · 4c-iii **CRMDashboardPage 3.867** (KPI hero `.kpi` + 5 warna + Oswald **tidak disentuh**; `git diff` blok itu wajib kosong) | build/lint per giliran; screenshot CRM Dashboard, Customer List/Detail, Quotation List/Form; `INQ_STAGE_COLOR` funnel tetap (status) | **YA** untuk ketiganya | ⛔ belum mulai |
| **4d — modul lain (±14)** | assets 4 + AddAsset/AssetShell, sales-order 3, inventory 2 + PenerimaanBarang, procurement PRFForm + 4, foundation OrgStructure, MyProfile, **Dashboard.jsx + HomeDashboard** | build/lint; screenshot Home, Dashboard, Asset Dashboard, Inventory Dashboard, PRF Form | Preview untuk Home/Dashboard | ⛔ belum mulai |
| **4e — Shell (`App.jsx` + global) = penutupan TD-70** | `App.jsx` **hanya** blok `PASTEL`, `NAV_TONES`, sidebar, `<style>` (cabut `@import` legacy; kelas `.font-display/.font-numeric/.font-mono` 18/39/49 pemakaian → `FONT_*`); `index.css` (`:root` → sage, cabut `@import` legacy, `.nx-*` tetap); `tailwind.config.js` `fontFamily` → 3 font; `index.html` tidak berubah (Oswald tetap). Bukan rewrite `App.jsx` | build/lint; screenshot shell (sidebar desktop + drawer 375px) di 4 modul; Network: nol request Google Fonts selain 4 font | **YA** — menyentuh semua halaman | ⛔ belum mulai |
| **4f — keluarga ke-5** | 50 file hex tanpa objek token + 194 kelas Tailwind palet (BNF, Meeting/Briefing, MOM, HRGA `hrga-tokens.js` `D` → kit, procurement, Home) — per modul | seperti 4d | Preview | ⛔ belum mulai |
| **4g — logistics non-web-Storbit (7)** | InputSP, SalesOrderPage, PickingList/Detail, DeliveryNote/Detail, PenerimaanBarang | screenshot SP List, Input SP, Picking, Surat Jalan | **YA** — ops Storbit harian | ⛔ belum mulai |
| **5 — Storbit web ungu/serif (#60 a)** | `SalesOrderDetailPage` 2.722 + `StorbitDashboardPage` 2.233: `C` ungu → token kit; objek `heading/body/mono` → `FONT_*`; **`salesOrderDetail.module.css` dihapus** (TTF `src/assets/fonts/` tetap — dipakai PDF); warna slice donut `spStatusConstants.js` **tetap** (kelas status). **Print kit & 4 PDF nol diff (gate).** | build/lint; screenshot Detail SP (header, tab Shipment/Items, panel Finance), Dashboard Storbit 3 tab; smoke buka SP + Generate Picking (jangan "Tandai Terkirim" di staging — TD-265) | **YA** | ⛔ belum mulai |
| **6 — TD-269 + TD-270** | Sisa `#2F6B3F`/`#E7EFE2` harus **nol** (grep gate) · emoji/simbol → Lucide (`App.jsx` toast `✨`/`🎉`, `AssetDetailITPage:1499/1501`, `✓`/`✗` di Quotation Form/List/Detail, DeliveryNote, InputSP, SalesOrderPage, HrgaRequestDetail, `StatusBar` `'✓'` → `<Check/>`) · TD-270: verifikasi 6 pemanggil `SaveButton` mengembalikan Promise & melempar saat gagal (perbaikannya masuk Batch DS 1–2) → tutup kedua TD | grep gate + lint; smoke alur simpan admin yang ditolak RLS (harus error, bukan "Tersimpan!") | `main` | ⛔ belum mulai |
| **7 — Bersih-bersih** | Hapus `admin-settings/kit.jsx`+`tokens.js`, alias kompatibilitas, `hrga-tokens.js` `D`; gate akhir: **nol** `#144682` / `#1B4D8A` / `#E85A1E` / `#E8703D` / `#F6EFE3` / `#FFFDF8` / `#F2F5F9` di luar print/PDF, nol `#5b3fa0` di `.jsx` web, nol font legacy, nol kelas `font-*` legacy; dokumen ini ditulis ulang jadi dokumen kit tunggal (§1–§18 diganti); doc-keeper | build/lint; grep gate dicetak di PROGRESS | `main` | ⛔ belum mulai |

**A.4.4 Protokol verifikasi (tiap Batch DS)**

| # | Gate |
|---|---|
| 1 | `npm run build` clean (jumlah modul dicatat) · `npm run lint` ≤ 160 (139/21) dan `npx eslint <file Batch DS>` tidak menambah error |
| 2 | `git diff --stat` **persis** daftar file Batch DS; gate keras setiap Batch DS: `git diff --stat -- src/modules/logistics/print* src/modules/logistics/*PDF.jsx src/modules/crm/QuotationPDF.jsx` **kosong**, dan blok `.kpi`/Oswald di `CRMDashboardPage` nol diff |
| 3 | Grep gate hex lama per Batch DS (angka turun; nol hex baru di luar `tokens.js`) |
| 4 | Screenshot **sebelum/sesudah**, desktop 1280 + mobile 375, halaman kunci: Home/App Launcher · CRM Dashboard · Inquiry List · Detail Deal · Pipeline · Customer List · Quotation Form · Storbit Dashboard · Detail SP · SP List · Picking/Surat Jalan · AdminHub · Companies · User Access · Entity Settings. "Sebelum" diambil sekali di Batch DS 1; disimpan di luar repo dan dikirim ke Den per Batch DS |
| 5 | Batch DS berlabel **YA**: branch → Vercel preview + DB staging → smoke test oleh Den (login) → baru merge `main` |
| 6 | Doc-keeper di akhir tiap Batch DS (PROGRESS + CLAUDE Recent + TD + kolom Status tabel A.4.3) |

**A.4.5 Risiko yang sudah diketahui**

| Risiko | Penanganan |
|---|---|
| Batch DS 3, 4c, 4e, 4g, 5 = halaman trafik tinggi (CRM sales harian, shell semua halaman, Storbit ops) | wajib preview + staging sebelum `main` |
| Staging DB tertinggal dari produksi (TD-265: `signed_date`/overload `mark_delivery_delivered` belum ada) | smoke Storbit di staging hindari "Tandai Terkirim", atau jalankan `20260917000001` di staging dulu (di luar rencana ini) |
| `SaveButton` berubah perilaku (menunggu server) — pemanggil yang `onSave`-nya tidak mengembalikan Promise akan tetap "sukses instan" | tiap pemanggil dicek di Batch DS 2 *[Batch DS 2 selesai: 6 pemanggil TD-270 + 4 alur simpan lain → `SaveButton` kit, handler mengembalikan `{ error }`/`false`; smoke staging: "Tersimpan!" 15–30 ms SETELAH respons PostgREST, offline → "Gagal menyimpan"; alur simpan 5 halaman master + Integrations sengaja tetap `PrimaryBtn` (sudah `await`/tanpa DB); ⚠️ HRGA "Simpan Semua" **BELUM LOLOS — belum pernah dites** (staging MSI nol `hrga_request_types`, tombol tak dirender) → **masuk daftar cek Batch DS 6** bersama smoke tolakan RLS; verifikasi resmi Batch DS 6]* |
| ~~Kartu ≈ halaman (1,05:1) → tampilan lebih "rata"~~ | ~~konsekuensi palet, bukan bug; kalau screenshot Batch DS 2 terasa kurang berhierarki, penyesuaian (mis. bayangan tipis turunan) = keputusan Den, bukan mengubah 7 hex~~ ✅ **DITUTUP keputusan Den 20 Sep 2026 malam (saat review Batch DS 2):** Den memilih jalan yang TIDAK disebut kolom ini — **mengubah satu hex kebijakan A.1 (`CARD` → `#FFFFFF`)**, bukan bayangan turunan; kartu vs halaman kini 1,07:1, dan pemisahnya = border `LINE` (1,35:1 vs kartu). Ini revisi kebijakan yang tercatat (A.1 catatan revisi), bukan konsekuensi palet lagi. Turunan `LINE_SOFT`/`HEAD_BG`/`DISABLED_BG` dihitung ulang dengan rumus yang sama (keputusan #1 tetap berlaku). |
| Env lokal | ✅ sejak 20 Sep 2026 `.env.local` & `.env.staging` menunjuk **staging** `oovmlhilhqzejnawqkvt` (bukan produksi lagi); kode membaca `VITE_SUPABASE_URL` + `VITE_SUPABASE_KEY` |

**A.4.6 Empat belas keputusan yang sudah dikonfirmasi Den (20 Sep 2026)**

1. **Token turunan diizinkan** — hover/disabled/fokus/backdrop/bayangan/`LINE_SOFT`/`ROW_HOVER` dihitung dari 9 hex kebijakan, tanpa hue baru, rumus tercatat di `tokens.js`. *[Dipraktikkan 20 Sep malam saat `CARD` direvisi (A.1): rumus tetap, nilai `LINE_SOFT`/`HEAD_BG`/`DISABLED_BG` dihitung ulang — turunan mengikuti hex kebijakan, bukan sebaliknya.]*
2. **Teks di atas aksen = `#33422B`** — tombol primer tampak "hijau muda berteks gelap" (putih gagal AA di semua aksen).
3. **Palet status/data** = trio semantik warm-beige yang sudah ada (ok/warn/danger/info), `TONE` v3, dan `DEAL_STATUS` 7 status **termasuk WON navy `#144682` sebagai warna STATUS** (bukan identitas); warna slice donut/funnel/chart = kelas status, tidak disentuh. *[Dipertajam #62, 20 Sep 2026: bg/bd trio tetap persis; EMPAT fg (`ok`/`warn`/`slate`/`orange`) digeser minimal supaya lolos AA 4,5:1 — nilai kit menang atas nilai warm-beige/v3 lama.]*
4. **Batch DS 4f masuk scope sekarang** (50 file hex + 194 kelas Tailwind) sebagai giliran terakhir Batch DS 4 — tanpa ini kit tunggal tidak tunggal.
5. **TD-270 dibangun benar sejak Batch DS 1** (`SaveButton` async); Batch DS 6 tinggal verifikasi & tutup TD.
6. **TD-70 (font legacy) + `index.css :root` + `tailwind.config.js` masuk Batch DS 4e** bersama `App.jsx`; Oswald tetap.
7. **Ukuran komponen tunggal**: tombol `md` 40px, input ringkas 44, input mengambang 56, radius tombol/input 10, kartu 14.
8. **`Icon name=…` registry dipertahankan** sebagai kompatibilitas; komponen baru impor Lucide langsung.
9. **`Chatter` v3 (0 pemakai) dan `CrmV3DemoPage` dihapus** di Batch DS 3; `InquiryChatter.jsx` tetap yang hidup.
10. **Kosakata CRM keluar dari kit** (`crm/dealStatus.js`); **`ENTITIES` tunggal** di `src/lib/entities.js`.
11. **Alur kerja**: branch per Batch DS (`feat/kit-ds1-tokens` …), satu commit per Batch DS, merge ke `main` setelah review Den; push hanya atas instruksi; doc-keeper tiap akhir Batch DS.
12. **Screenshot** disimpan di luar repo dan dikirim per Batch DS, tidak di-commit.
13. **Env staging** sudah dibereskan 20 Sep 2026 (`.env.local`/`.env.staging` → `nexus-staging`), prasyarat Batch DS bertanda YA terpenuhi.
14. **Warna "Grand Total" `#82480F`** di Detail SP/Dashboard Storbit diperlakukan sebagai **penekanan** (`INK` tebal + tint `ACCENT_2`), bukan status.
...
| Halaman **Foundation / Admin Settings / Master Data** (`AdminHub` dan turunannya) | **Kit tunggal `src/kit/`** (A.4.2) — ~~**AdminKit** `src/pages/foundation/admin-settings/` (§6.2)~~ | ✅ **[Batch DS 2 selesai — review ditutup Den 20 Sep 2026 malam]** ke-22 file area ini *[21 sejak 21 Sep 2026 — `AdminSettingsHub.jsx` dihapus Batch FS Fase 1]* memakai **kit tunggal `src/kit/`** (impor `…/kit` + `…/kit/tokens`; `kit.css` lewat `main.jsx:4`); AdminKit lama = **kode mati** (nol pemakai; file tetap ada s/d Batch DS 7 — jangan diimpor lagi). Halaman admin/Foundation BARU ikut `src/kit/` — satu-satunya area yang gilirannya sudah lewat (#61 berhenti per halaman). Kalimat "kit resmi" di TD-14 (`08_TECH_DEBT.md`) kini menunjuk `src/kit/`. |
...
| **Kit tunggal `src/kit/` + `src/lib/entities.js`** (lahir Batch DS 1, 20 Sep 2026 malam) | ⛔ **JANGAN dipakai halaman apa pun di luar gilirannya — pemakainya hari ini = 22 halaman admin/Foundation + `main.jsx` (Batch DS 2, selesai 20 Sep 2026 malam) *[21 halaman sejak 21 Sep 2026 — `AdminSettingsHub.jsx` dihapus Batch FS Fase 1]*; halaman lain tetap dilarang.** Halaman masuk kit hanya lewat giliran Batch DS-nya (A.4.3: Batch DS 2 = 22 file AdminKit + `main.jsx` impor `kit.css` — ✅ sudah; Batch DS 3 = CRM v3; dst.), masing-masing atas instruksi Den. | #61 tetap berlaku — **FINAL, dikonfirmasi Den 20 Sep 2026 (sesi keenam)**: "kit ada" ≠ "instruksi implementasi turun" untuk semua halaman; berhenti per halaman saat Batch DS-nya selesai. Mengimpor `src/kit` sepihak = migrasi di luar rencana, dua kit di satu halaman, dan screenshot "sebelum/sesudah" per Batch DS jadi tak bermakna. Isi kit: A.4.2 + §1 (baris `src/kit/`). |
...
|---|---|---|---|
| `src/pages/foundation/admin-settings/tokens.js` (37 baris) | 14 Jun 2026 | 14 konstanta warna, 3 font, `ENTITIES`, `fmtRp`/`fmtNum` | ~~22 file: 11 halaman `admin-settings/` + `AdminHub.jsx` + 10 halaman master-data `src/modules/admin/pages/`~~ **NOL pemakai di luar `kit.jsx` sendiri sejak Batch DS 2 (selesai 20 Sep 2026 malam)** — ke-22 file pindah ke `src/kit/tokens` (diverifikasi grep doc-keeper sesi kesembilan: rujukan `admin-settings/tokens` tersisa = komentar `src/lib/entities.js:6` & `crm/v3/tokens.js:5`); file ini tetap ada s/d Batch DS 7 |
| `src/pages/foundation/admin-settings/kit.jsx` (573 baris) | 14 Jun 2026 | **24 export** (komentar `crm/v3/kit.jsx:4` yang menyebut "20 export" sudah basi): `Icon`, `KitStyles`, `PageHeader`, `SectionLabel`, `EntitySwitcher`, `Tabs`, `FloatingInput`, `FloatingSelect`, `Toggle`, `NumberStepper`, `Segmented`, `PrimaryBtn`, `OutlineBtn`, `SaveButton`, `Tooltip`, `SlideOver`, `Modal`, `DropZone`, `UploadBox`, `useToast`, `Skel`, `Card`, `KitSelect`, `PillToggle` | sama seperti di atas — **nol pemakai sejak Batch DS 2** (`<KitStyles/>` dicabut 17×, diganti `kit.css` di `main.jsx:4`; `SaveButton` lama tak lagi dipanggil siapa pun; rujukan tersisa = komentar `CompanySwitcher.jsx:10` & `crm/v3/kit.jsx:4`) |
...
| **`src/kit/`** (12 file, ~~2.335~~ ~~2.345~~ **2.347** baris — `wc -l` sesudah revisi `CARD` 20 Sep malam, sesi kedelapan): `tokens.js` (~~166~~ ~~175~~ **177**) · `kit.css` (56) · `Button.jsx` (245) · `Form.jsx` (372) · `Layout.jsx` (268) · `Data.jsx` (431) · `Overlay.jsx` (154) · `Upload.jsx` (108) · `Icon.jsx` (64) · `useToast.jsx` (56) · `index.js` (~~31~~ **32**) · `__demo/KitGallery.jsx` (384) | 20 Sep 2026 (Batch DS 1) | **Kit tunggal ber-palet sage + tiga font** (A.4.2): 9 hex A.1 + turunan berumus; palet STATUS/DATA kelas terpisah (`SEMANTIC`/`TONE`/`DEAL_STATUS`; sejak #62 [20 Sep, sesi keenam] empat fg `ok`/`warn`/`slate`/`orange` digeser minimal ke AA 4,5:1 — seluruh pasangan teks/status kit lolos AA); **31 export** (komponen + `Icon` + `useToast`) + **5 alias** nama lama (`PrimaryBtn`/`OutlineBtn`/`KitSelect`/`Skel`/`Tabs`, dicabut Batch DS 7); `FormSheet`/`Notebook`/`StatusBar`/`ListView` = **salinan v3** berwarna sage (file v3 asli tak disentuh, dihapus Batch DS 3); `Icon` registri 66 nama identik AdminKit; `kit.css` prefiks `nk-` (≠ `ak-`); `SaveButton` menunggu Promise (arah TD-270) | ~~**Di `main`/produksi: NOL — KODE MATI.**~~ **22 file (Batch DS 2, selesai 20 Sep 2026 malam — diverifikasi ulang doc-keeper sesi kesembilan):** 11 `admin-settings/*` + `AdminHub.jsx` + 10 `modules/admin/pages/*` mengimpor `…/kit` + `…/kit/tokens`; `main.jsx:4` mengimpor `kit.css`; build 2.646 modul (+10 = kit masuk graph; etalase tetap di luar, tidak di-route). Satu-satunya `from '../kit'` lain di luar 22 itu = `crm/v3/__demo/CrmV3DemoPage.jsx:24` → `crm/v3/kit.jsx` (bukan kit tunggal). Tampilan 22 halaman itu berubah begitu deploy `main` berikutnya; halaman lain belum. |
| `src/lib/entities.js` (39 baris) | 20 Sep 2026 (Batch DS 1) | `ENTITIES` tunggal `{ code, id: UUID companies.id, name legal }` (SOA "PT Stuja Orbit Abadi") + `ENTITY_CODES` + `ENTITY_BY_CODE`/`ENTITY_BY_ID` (frozen) + `entityByCode`/`entityById`/`entityCodeOf` | `src/kit/Form.jsx` (`EntitySwitcher`, banding `e.code`) + **5 halaman admin-settings** (ApprovalWorkflows · DocumentSettings · EntitySettings · FinanceDefaults · Notifications) lewat `ENTITY_IDS = Object.fromEntries(ENTITIES.map(e => [e.code, e.id]))` *[Batch DS 2, selesai]*. Dua versi lama tetap hidup s/d Batch DS 7 (`admin-settings/tokens.js:29` id = kode tanpa UUID · `crm/v3/tokens.js:147` id = UUID); versi lokal lain yang juga harus dilebur: `PositionsPage.jsx:46` (UUID, nama tanpa "PT" — ~~Batch DS 2~~ ⚠️ **TIDAK disentuh Batch DS 2** [`main` `:40-45`, `ENTITY_IDS = ENTITIES.map(e => e.id)` untuk `.in('company_id', …)`, beda bentuk; `handleSave` byte-identik per TD-14] — giliran barunya perlu konfirmasi Den: Batch DS 7 bersama dua versi lama, atau lebih awal) · **`CRMReportPage.jsx:61`** `["MSI","JCI","SOA"]` (array kode — *temuan doc-keeper 20 Sep malam, belum disebut rencana; kandidat Batch DS 4c*) |
...
**Resmi sejak 19 Sep 2026 (Bagian A.1; CARD direvisi 20 Sep malam):** latar halaman `#F6F8F3` · latar kartu `#FFFFFF` (semula `#EEF3EA`) · aksen utama `#C3D9B8` (hover `#AFC9A0`) · aksen kedua `#D7C9B0` (hover `#C9B896`) · border `#D8E0D2` · teks utama `#33422B` · teks sekunder `#5C6B52`. ~~**Nol kemunculan** ketujuh hex ini di `src/` hari ini.~~ **[20 Sep 2026 malam, Batch DS 1]** Ketujuh hex kini ada di **`src/kit/tokens.js:31-39`** *(`:29-37` sebelum revisi `CARD` menambah 2 baris komentar header)* (+ cermin `kit.css:19-22`) — **dan HANYA di sana**: nol kemunculan di luar `src/kit/` *[⚠️ sejak `CARD` = `#FFFFFF` (sesi kedelapan) klaim ini berlaku untuk **ENAM hex sage khas**; `#FFFFFF` sebagai putih biasa ada di **35 file** lain di `src/` dan **bukan penanda grep** — diukur doc-keeper]* — tetap benar sesudah Batch DS 2 selesai (22 halaman admin memakai NAMA token, nol literal sage di halaman; diverifikasi doc-keeper), jadi yang menampilkannya di `main` = 22 halaman admin/Foundation itu saja (Batch DS 2, review ditutup Den 20 Sep 2026 malam); halaman lain belum.
...
| Cream | `#F6EFE3` | `#F6EFE3` | — | header/thead `HEAD_BG #EFF3ED` (latar kartu kini putih) |
| Surface kartu | `#FFFFFF` | `#FFFDF8` | `#FFFFFF` | latar kartu `#FFFFFF` (revisi 20 Sep malam) |
...
Sebaran palet lama di `src/` (file / kemunculan, case-insensitive; `main` @ 19 Sep): `#144682` **23 / 36** · `#1B4D8A` **54 / 123** · `#E85A1E` **67 / 108** · `#E8703D` **9 / 10** *[Batch DS 2 (selesai) — jumlah FILE diukur ulang doc-keeper 20 Sep malam: `#1B4D8A` **52** (lepas dari `AuditLogPage`, `DropdownManagementPage`) · `#E85A1E` **66** (lepas dari `DropdownManagementPage`) · `#144682` 24 (= 23 + `src/kit/tokens.js` sebagai STATUS) · `#E8703D` 9 — turunnya kecil karena 22 halaman AdminKit memakai NAMA token `NAVY`/`ORANGE` dari `admin-settings/tokens.js`, bukan literal; literalnya baru hilang saat file token lama dihapus di Batch DS 7]* · `#c44d18` 1 / 1 · `#D14E18` 5 / 6. Keempat hex itu **sudah bukan warna aplikasi resmi**, tapi masih di kode — jangan "dirapikan" sepihak (§17.1); penggantiannya = penyatuan kit.
...
⚠️ *Tabel di atas = `crm/v3/tokens.js` apa adanya (masih dipakai 3 halaman v3). Kit tunggal `src/kit/tokens.js` `TONE` memakai bg/bd yang sama tetapi fg `slate` **`#676D7B`** dan `orange` **`#BE4A19`** (digeser minimal demi AA, Keputusan #62, 20 Sep 2026) — jangan menyalin fg v3 ke kit; dua salinan sementara sampai Batch DS 3, yang di kit yang berlaku ke depan.*
...
⚠️ *Nilai `ok`/`warn` di atas = file lama apa adanya. Kit tunggal `src/kit/tokens.js` `SEMANTIC` memakai bg/bd yang sama tetapi fg `ok` **`#2D794D`** dan `warn` **`#8E620D`** (digeser minimal demi AA 4,5:1 teks badge, Keputusan #62, 20 Sep 2026) — file lama ikut nilai kit pada giliran Batch DS-nya.*
...
### 6.2 AdminKit — `src/pages/foundation/admin-settings/` *(⚠️ ARSIP sejak Batch DS 2 selesai 20 Sep 2026 malam: kit ini **nol pemakai** di `main` — ke-22 halamannya memakai `src/kit/`, `<KitStyles/>` tak dirender lagi; file kit tetap ada s/d Batch DS 7. Tabel di bawah = kontrak lama, jangan dipakai untuk halaman baru)*

Import: `import { PageHeader, Card, FloatingInput, … } from '../../../pages/foundation/admin-settings/kit'` dan token dari `…/tokens`. **`<KitStyles />` wajib dirender sekali di halaman** (keyframes, `.ak-skel`, `.ak-spin`, `.ak-scroll`, `.ak-rise`). *[Batch DS 2 selesai: ke-22 halaman tidak lagi merender `<KitStyles/>` (dicabut 17×) — gantinya `kit.css` diimpor sekali di `main.jsx:4`, kelas `nk-*`.]*
...
| `SaveButton` | tombol simpan 3-keadaan idle→saving→saved | `onSave` `label="Simpan"` `variant="primary"/"navy"`; ⚠️ **tidak menunggu Promise** — `onSave` dipanggil setelah jeda 950ms, "Tersimpan!" 2s lalu reset, apa pun hasil server (**TD-270**; *sejak Batch DS 2 keenam pemanggilnya memakai `SaveButton` kit `src/kit/Button.jsx` yang menunggu Promise — komponen lama ini tak berubah dan nol pemakai; TD tetap OPEN s/d Batch DS 6*) | `<SaveButton onSave={handleSave}/>` |
...
> ⚠️ **Dua lapis:** target akhirnya = palet sage + tiga font + kit tunggal (Bagian A). **Selama penyatuan belum dieksekusi** (status hari ini), langkah 1–2 memakai kit/token yang **ada** supaya tidak lahir palet kelima (#61, DISETUJUI Den 20 Sep 2026). Kit tunggal kini **ADA** (`src/kit/`, Batch DS 1) — tetapi #61 **FINAL** (Den, 20 Sep 2026 sesi keenam): langkah 1–2 **tetap** memakai kit/token yang ada untuk halaman yang belum tiba giliran Batch DS-nya; baru **pada giliran Batch DS halaman itu** langkah 1–2 diganti "pakai `src/kit/tokens.js`" dan sisanya tetap berlaku. Kalau mockup datang dalam warna sage (mengikuti kebijakan), **petakan ke token kit yang ada** lewat kolom "padanan sage" §3.1 (kebalikannya) — jangan menyalin hex sage-nya ke kode sebelum instruksi.
...
12. **Kontras:** teks ≤14px wajib ≥4.5:1 (preseden: dua hex STATUS_META digeser demi AA 2 Sep 2026; empat fg kelas STATUS kit tunggal digeser demi AA 20 Sep 2026 — #62; `BY MSI` 3.55:1 tercatat TD-227).
...
## 17. Temuan audit — divergensi & keputusan terbuka (kode halaman tidak diubah; status per 19 Sep 2026 di tiap butir, + anotasi Batch DS 1 [20 Sep malam] di butir 4/8/9/10)

> **Butir 1–2 sudah DIJAWAB oleh keputusan 19 Sep 2026** (palet sage menggantikan semua navy/oranye — Bagian A); **butir 3** dijawab sebagian (tiga font resmi; pembersihan kode = TD-70, ditunda); **butir 4 → TD-269**, bagian `SaveButton` di **butir 9 → TD-270** — semuanya **BELUM diperbaiki di kode, sengaja ditunda menunggu instruksi Den**. Sisanya tetap temuan terbuka. **[20 Sep 2026 malam — Batch DS 1]** Kit tunggal lahir sebagai kode mati (`src/kit/`): ia **membangun jalan** untuk butir 4/8/9/10 (`<Check/>` Lucide, `tokens.js` bersama, `SaveButton` ber-`await`, celah v3 tertutup di salinan) **tanpa menyentuh satu pun file yang ditemukan bermasalah** — status TD-269/TD-270 dan seluruh butir di bawah **tidak berubah**; anotasi per butir menandai apa yang sudah ada di kit. **[20 Sep 2026 malam — Batch DS 2 SELESAI, review ditutup Den (sesi kesembilan; dikerjakan di branch `feat/kit-ds2-adminkit`)]** 22 halaman AdminKit + `main.jsx` pindah ke kit tunggal (A.4.3): butir 9 bagian `SaveButton` **bergerak** (6 pemanggil kini `SaveButton` kit + handler `return { error }`; smoke staging "Tersimpan!" sesudah respons server) tapi **TD-270 tetap OPEN** sampai verifikasi resmi Batch DS 6 (HRGA "Simpan Semua" belum teruji — masuk daftar cek); butir 8 (11 file `PASTEL`) **tidak berubah** — `AdminFormModal`/`Loading|Empty|ErrorState` sengaja tak disentuh (Batch DS 4b); butir 1–2 di 22 halaman itu sudah tak berlaku (nol navy/oranye), di halaman lain masih.
...
8. **52 file** memakai objek token lokal (`const C/D/S…`) dan **11 file** masih `const PASTEL` (TD-14 PARTIAL: `App.jsx`, `AuthGate`, `CompanySwitcher`, `ErrorBoundary`, `AdminFormModal`, `AdminPageHeader`, `EmptyState`, `ErrorState`, `LoadingState`, `userAccessTokens.js`, `Dashboard.jsx`). Nilai-nilainya hampir seragam (§3.4) sehingga migrasi ke token bersama mungkin; token bersamanya = ~~`tokens.js` sage kit tunggal (A.3) — belum ada, BELUM DIKERJAKAN~~ **`src/kit/tokens.js` — ADA sejak Batch DS 1 (20 Sep malam) sebagai kode mati**; migrasi ke-52 + 11 file itu = Batch DS 4 (4a–4g), **BELUM DIKERJAKAN**.
9. **Celah kontrak AdminKit** yang sudah tercatat di TD-14: `OutlineBtn` & `FloatingSelect` tanpa `disabled`; `FloatingInput` tanpa `maxLength`; `Toggle` hanya oranye; `SaveButton` tidak menunggu Promise (menampilkan "Tersimpan!" 950 ms setelah klik apa pun hasilnya — **TD-270**, OPEN, ditunda; 6 pemakai di Admin Settings); `EntitySwitcher` single-select dengan pil selalu navy. *[20 Sep malam, Batch DS 1: seluruh celah ini tertutup **di kit BARU saja** — `src/kit/Form.jsx` (`disabled`/`maxLength`/`error`, `Toggle` sage, `EntitySwitcher` banding `code`) dan `src/kit/Button.jsx:123-162` (`SaveButton` `await onSave()`; throw / `{ error }` truthy → GAGAL + `onError`; `false` → idle; terverifikasi headless di etalase). AdminKit `kit.jsx` **tidak berubah**, 6 pemakainya masih memakai versi lama → **TD-270 tetap OPEN** sampai Batch DS 2 memindahkan pemakainya dan Batch DS 6 menutup.]* *[20 Sep malam, Batch DS 2 — SELESAI, review ditutup Den sesi kesembilan: keenam pemakai (+4 alur simpan lain) **sudah dipindahkan** ke `SaveButton` kit dengan handler `return { error }`/`return { error: e }`/`return false`; smoke staging 5 simpan sukses "Tersimpan!" 15–30 ms SETELAH respons PostgREST, offline → "Gagal menyimpan"; ⚠️ HRGA "Simpan Semua" **belum teruji — BUKAN lolos** (masuk daftar cek Batch DS 6); `OutlineBtn`/`FloatingSelect` di halaman itu kini alias/komponen kit yang punya `disabled` → **TD-270 tetap OPEN** (verifikasi resmi + smoke tolakan RLS + HRGA = Batch DS 6).]*
10. **Celah v3**: input search/textarea tanpa gaya fokus; `PrimaryBtn`/`OutlineBtn` tanpa hover; `Chatter` nol pemakai produksi & satu adapter; tiga hex papan (`#FBFAF9`, `#ECE7E2`, radius 16) sengaja belum jadi token. *[20 Sep malam, Batch DS 1: salinan v3 di `src/kit/` menutup tiga yang pertama — `SearchInput` bergaya fokus, `Button` ber-hover, tiga hex papan → `INPUT_BG`/`LINE_SOFT`/`RADIUS.lg`; file v3 asli **tidak disentuh** (dihapus Batch DS 3), `Chatter` v3 tetap nol pemakai.]*
...
// AdminKit (halaman Foundation / master data) — ARSIP: sejak Batch DS 2 selesai (20 Sep 2026 malam) ke-22 halaman ini memakai bentuk "Kit tunggal" di bawah; bentuk ini nol pemakai — jangan dipakai lagi
...
// Kit tunggal (src/kit/) — lahir Batch DS 1 (20 Sep 2026); sejak Batch DS 2 selesai dipakai 22 halaman admin/Foundation: JANGAN diimpor halaman LAIN mana pun
// di luar giliran Batch DS-nya (A.4.3, #61 — FINAL, dikonfirmasi Den 20 Sep 2026). Bentuk impor yang dipakai Batch DS 2 (22 halaman) dan Batch DS 3+ (path relatif —
// repo TIDAK punya alias `@` — selalu path relatif):
import { Button, SaveButton, IconButton, Card, PageHeader, FormSheet, Notebook, ListView, StatusBar,
         Badge, DocNo, EmptyState, Skeleton, FloatingInput, Select, Toggle, EntitySwitcher, Modal,
         SlideOver, Tooltip, Icon, useToast } from '../../kit';            // + alias sementara: PrimaryBtn, OutlineBtn, KitSelect, Skel, Tabs
import { BG, CARD, ACCENT, ACCENT_2, LINE, INK, INK_SOFT, INK_FAINT, FONT_HEAD, FONT_BODY, FONT_MONO,
         SP, RADIUS, SIZE, SEMANTIC, TONE, DEAL_STATUS, toneOf, closedFrom } from '../../kit/tokens';
import { ENTITIES, entityByCode } from '../../lib/entities';
// `kit.css` diimpor SEKALI di src/main.jsx (Batch DS 2 selesai — `main.jsx:4`) — bukan per halaman, bukan <KitStyles/>.
// Bersama
